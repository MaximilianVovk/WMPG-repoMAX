...
def extract_other_prop(input_dirfile, output_dir_show):
    ...
    norm_m = Normalize(vmin=np.min(m_init), vmax=np.max(m_init))
    ...
    scatter = plt.scatter(eeum, eeum_end, c=rho_total, cmap='viridis', s=30,
                            norm=norm, zorder=2)
    # create a scatter wth empty circles for the m_init
    scatter_m_init = plt.scatter(eeum, eeum_end, c=m_init, cmap='coolwarm_r', s=80,
                            norm=norm_m, zorder=1)
    # add a colorbar for the m_init in log scale
    plt.colorbar(scatter_m_init, label=r'm_{init} (kg)', ticks=np.linspace(np.min(m_init), np.max(m_init), 5))
    plt.colorbar(scatter, label='Rho (kg/m³)')
    ...
    scatter = plt.scatter(eeucs, eeucs_end, c=rho_total, cmap='viridis', s=30,
                            norm=norm, zorder=2)
    # create a scatter wth empty circles for the m_init
    scatter_m_init = plt.scatter(eeucs, eeucs_end, c=m_init, cmap='coolwarm_r', s=80,
                            norm=norm_m, zorder=1)
    # add a colorbar for the m_init in log scale
    plt.colorbar(scatter_m_init, label=r'm_{init} (kg)', ticks=np.linspace(np.min(m_init), np.max(m_init), 5))
    # add a colorbar for the rho_total
    plt.colorbar(scatter, label='Rho (kg/m³)')
