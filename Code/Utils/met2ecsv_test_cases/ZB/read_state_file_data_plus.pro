pro read_state_file_data_plus,meteor0,dir,stfl,seq,time,ts0,mir_site1,mir_site2


;program to read state file and output t,L,
...
mir_site1={mir_site_data,site:0,ts:0L,tu:0L,lat:0.0,lon:0.0,elev:0.0,th0:0.0,phi0:0.0,rot:0.0,wid:0,ht:0,offset:0.0,site_name:'',starcat:''}
meteor={meteor_data,tag:0,site:0,fr:0,id:0,cx:0.0,cy:0.0,rx:0.0,ry:0.0,px:0,py:0,rad:0,qp:0,th:0.0,phi:0.0,th_err:0.0,tau:0.0,lsp:0.0,mag:0.0,mag_err:0.0,$
          cr:0,cg:0,cb:0,star:0,ts:0L,tu:0L,ox:0,oy:0,pos:0.0,off:0.0,type:'',t:0.0,L:0.0,R:0.0,ht:0.0,vel:0.0,rv:0.0,rh:0.0,lat:0.0,lon:0.0,sc:0,no:0}

mir_site2=mir_site1

if n_elements(stfl) eq 0 then stfl=rmd_pickfile(FILTER_IN='*.met',Title='Select state file ')
...
while not eof(inunit) do begin
        ...
        ;print,line
       S0 = STRSPLIT(line, ' ', /EXTRACT)
if (s0[0] eq 'video' and s0[3] eq '1') or  (s0[0] eq 'video' and s0[3] eq '4') then begin
 print,line
;mir_site1.event=s0[7]
mir_site1.site=s0[where(S0 eq 'site' ,count)+1]
mir_site1.ts=long(s0[where(S0 eq 'ts' ,count)+1])
mir_site1.tu=long(s0[where(S0 eq 'tu' ,count)+1])
mir_site1.lat=float(s0[where(S0 eq 'lat',count)+1])
mir_site1.lon=float(s0[where(S0 eq 'lon',count)+1])
mir_site1.elev=float(s0[where(S0 eq 'elv',count)+1])
mir_site1.th0=float(s0[where(S0 eq 'th0',count)+1])
mir_site1.phi0=float(s0[where(S0 eq 'phi0',count)+1])
...
mir_site2.ts=long(s0[where(S0 eq 'ts' ,count)+1])
mir_site2.tu=long(s0[where(S0 eq 'tu' ,count)+1])
mir_site2.lat=float(s0[where(S0 eq 'lat',count)+1])
mir_site2.lon=float(s0[where(S0 eq 'lon',count)+1])
mir_site2.elev=float(s0[where(S0 eq 'elv',count)+1])
mir_site2.th0=float(s0[where(S0 eq 'th0',count)+1])
mir_site2.phi0=float(s0[where(S0 eq 'phi0',count)+1])
...
if s0[0] eq 'frame' then begin
    ...
    endwhile
;stop
for j=0,n_elements(tag_names(meteor))-1 do begin
    ind=where(strupcase(S0) eq strupcase((tag_names(meteor))[j]) ,count)
    ;if count gt 0 then meteor.(j)=s0[ind+1]
    if count gt 0 then begin
    if s0[61] eq 'meteor' then meteor.(j)=s0[ind+1]
    endif
endfor

if count gt 0 then begin
if k eq 0 and s0[61] eq 'meteor' then begin
meteor0=meteor
k=k+1
endif else begin
if s0[61] eq 'meteor' then meteor0=[meteor0,meteor]
endelse
endif
i+=1
