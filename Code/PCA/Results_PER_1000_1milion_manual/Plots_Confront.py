...
only_select_meteors_from='20220820_055202A'
...
for current_shower in Shower:
    ...
    if only_select_meteors_from in df_obs['solution_id'].values:
        ...
        # delete the 'A' at the end of the solution_id_dist
        only_select_meteors_from=only_select_meteors_from[:-1]
        print('Observed event : '+only_select_meteors_from)
        
        # check if there is a .json file in the folder with the same name as only_select_meteors_from
        if os.path.isfile(os.getcwd()+r'\\'+only_select_meteors_from+'_sim_fit.json'):
                with open(os.getcwd()+r'\\'+only_select_meteors_from+'_sim_fit.json','r') as json_file:
                    data_json_fileManual = json.load(json_file)
                    fileManual=[]
                ...
                for key in mass_keys:
                    fileManual.append(data_json_fileManual[key])
                not_found_json=1
    else:
...
for current_shower in Shower:
    ...
    for i in range(3):
        for j in range(3):
            ...
            if not_found_json!=0:
                # change the title
                fig.suptitle(only_select_meteors_from)
                # add the mass_keys data
                if plotvar == 'mass':
                    axs[i,j].axvline(data_json_fileManual['m_init'], color='r', linewidth=3)
                elif plotvar == 'erosion_mass_min':
                    axs[i,j].axvline(data_json_fileManual['erosion_mass_min'], color='r', linewidth=3)
                elif plotvar == 'erosion_mass_max':
                    axs[i,j].axvline(data_json_fileManual['erosion_mass_max'], color='r', linewidth=3)
                elif plotvar == 'erosion_energy_per_unit_cross_section':
                    axs[i,j].axvline(data_json_fileManual['erosion_energy_per_unit_cross_section'], color='r', linewidth=3)
                elif plotvar == 'erosion_energy_per_unit_mass':
                    axs[i,j].axvline(data_json_fileManual['erosion_energy_per_unit_mass'], color='r', linewidth=3) #linestyle='dashed', 
                elif plotvar == 'rho':
                    axs[i,j].axvline(data_json_fileManual['rho'], color='r', linewidth=3)
                elif plotvar == 'sigma':
                    axs[i,j].axvline(data_json_fileManual['sigma']*1000000, color='r', linewidth=3)
                elif plotvar == 'erosion_height_start':
                    axs[i,j].axvline(data_json_fileManual['erosion_height_start']/1000, color='r', linewidth=3)
                elif plotvar == 'erosion_coeff':
                    axs[i,j].axvline(data_json_fileManual['erosion_coeff']*1000000, color='r', linewidth=3)
                elif plotvar == 'erosion_mass_index':
                    axs[i,j].axvline(data_json_fileManual['erosion_mass_index'], color='r', linewidth=3)
    ...
    figManager = plt.get_current_fig_manager()
    figManager.window.showMaximized()
    # add st.write(fig) to show the plot in streamlit
    plt.show()
