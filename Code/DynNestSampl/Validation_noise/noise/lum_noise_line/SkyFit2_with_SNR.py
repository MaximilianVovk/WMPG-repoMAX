...
class PlateTool(QtWidgets.QMainWindow):
    ...
    def loadState(self, dir_path, state_name, beginning_time=None, mask=None):
        ...
        # Once the state is fully loaded, call the function to save SNR values:
        self.writeSNRValuesToFile(dir_path)

        # Update the FPS if forced
        if hasattr(self, 'fps'):
            self.setFPS()

    def writeSNRValuesToFile(self, dir_path):
        """
        Writes a simple text file containing the frame number and SNR for each meteor pick
        in self.pick_list.
        """
        ...
        if "elgin" in dir_path:
            namefile = namefile+"elgin_"
        ...
        with open(out_file, "w") as f:
            ...
            for frame, pick in sorted(self.pick_list.items()):
                # Only write lines where a centroid actually exists:
                if pick.get("x_centroid") is not None and pick.get("snr") is not None:
                    x_c = pick["x_centroid"]
                    y_c = pick["y_centroid"]
                    snr = pick["snr"]
                    logsnr = np.log10(snr)
                    if x_c is None or snr is None:
                        # Skip if no star was actually picked or we have no SNR
                        continue

                    # Safeguard against missing intensity:
                    intensity_sum = pick.get("intensity_sum", 1)

                    # If you measure ground points, you may want to disable magnitude calls:
                    pp_tmp = copy.deepcopy(self.platepar)
                    if self.meas_ground_points:
                        pp_tmp.switchToGroundPicks()

                    # Build the time array for xyToRaDecPP:
                    time_data = [self.img_handle.currentFrameTime(frame_no=frame)]

                    # This call returns a "mag_data" list (the 4th returned value)
                    # using your existing photometry logic:
                    jd_data, ra_data, dec_data, mag_data = xyToRaDecPP(
                        time_data,
                        [x_c],
                        [y_c],
                        [intensity_sum],
                        pp_tmp,
                        measurement=True  # means it will treat these picks as measured positions
                    )

                    # We'll just take the first (only) magnitude value:
                    mag = mag_data[0]

                    f.write(f"{frame}, {snr:.2f}, {logsnr:.2f}, {mag:+7.2f}\n")

        print(f"[INFO] Wrote SNR values to {out_file}")
