...
# Penalty factor missmatch time or height sigma 
PENALITY_FACTOR = 1  # Define the penalty factor
# Penalty thresholds
TIME_THRESHOLD = 0.05  # seconds
HEIGHT_THRESHOLD = 2  # km
...
def fit_mag_polin2_stdev(data_mag, time_data):
    ...
    mag_pol_stdev = np.std(residuals_pol)

    return fit1, residuals_pol, mag_pol_stdev, 'Polinomial Fit'

def fit_lag_t0_stdev(lag_data, time_data, velocity_data):
    ...
    # Choose the best fitted velocity based on sigma
    ...
    # residuals_t0 = lag_data - cubic_lag(np.array(time_data), best_a_t0, best_b_t0, c_t0, best_t0)
    rmsd_t0 = np.sqrt(np.mean(residuals_t0 ** 2))
    lag_t0_stdev = np.std(residuals_t0)

    # plot the lag and the velocity results
    fig, ax = plt.subplots(1, 2)
    # use matplotlib to plot the data
    ax = ax.flatten()
    ax[0].plot(time_data, lag_data, 'go', label='Observation')
    ax[0].plot(time_data, fitted_lag_t0, 'k--', label='Cubic Fit')
    ax[0].set_xlabel('Time (s)')
    ax[0].set_ylabel('Lag (m)')
    ax[0].grid(True)
    ax[0].legend()
    ax[0].set_title('Lag Fit')

    ax[1].plot(time_data, velocity_data, 'go', label='Observation')
    ax[1].plot(time_data, best_fitted_vel_t0, 'k--', label='Cubic Fit')
    ax[1].set_xlabel('Time (s)')
    ax[1].set_ylabel('Velocity (m/s)')
    ax[1].grid(True)
    ax[1].legend()
    ax[1].set_title('Velocity Fit')
    ...
    return fitted_lag_t0, residuals_t0, lag_t0_stdev, 'Cubic Fit', best_fitted_vel_t0, fitted_acc_t0
...
def find_noise_of_data(data, fps=32, plot_case=False):
    ...
    fitted_lag_t0_lag, residuals_t0_lag, stdev_t0_lag, fit_type_lag, fitted_vel_t0, fitted_acc_t0 = fit_lag_t0_stdev(data_obs['lag'],data_obs['time'], data_obs['velocities'])
    # now do it for fit_mag_polin2_stdev
    fit_pol_mag, residuals_pol_mag, stdev_pol_mag, fit_type_mag = fit_mag_polin2_stdev(data_obs['absolute_magnitudes'],data_obs['time'])
    ...
    # print(len_t0_extr)
    print('stdev mag',stdev_pol_mag)
    print('stddev lag',stdev_t0_lag/1000)
    print('stddev vel',stdev_t0_lag*np.sqrt(2)/(1.0/fps)/1000)

    # compute teh RMSD of the of lag and magnitude
    print('RMSD mag',np.sqrt(np.mean(residuals_pol_mag ** 2)))
    print('RMSD lag',np.sqrt(np.mean(residuals_t0_lag ** 2))/1000)
    print('RMSD vel',np.sqrt(np.mean(residuals_t0_lag ** 2))*np.sqrt(2)/(1.0/fps)/1000)
    ...
    fit_funct = {
        ...
        'length': len_t0_extr,
        'stdev_lag' : stdev_t0_lag/1000,
        'stdev_mag' : stdev_pol_mag,
        'stdev_vel' : stdev_t0_lag/1000*np.sqrt(2)/(1.0/fps),
        'fps' : fps
    }
    ...
    if plot_case:
        ...
        return stdev_t0_lag, stdev_pol_mag, fit_pol_mag, fitted_lag_t0_lag, fit_funct, fig, ax
    else:
        return stdev_t0_lag, stdev_pol_mag, fit_pol_mag, fitted_lag_t0_lag, fit_funct
...
def generate_observation_realization(data, rmsd_lag, rmsd_mag, fit_funct, name='', fps=32, fig='', ax='', plot_case=False):
    ...
    pd_datfram_PCA = array_to_pd_dataframe_PCA(data_obs, fit_funct)
...
def read_GenerateSimulations_output_to_PCA(file_path, name='', fit_funct=''):
    if name!='':   
        print(name) 
    ...
    if gensim_data is None:
        ...
        pd_datfram_PCA = array_to_pd_dataframe_PCA(gensim_data, fit_funct)
        return pd_datfram_PCA
...
def array_to_pd_dataframe_PCA(data, fit_funct):
    ...
    sigma_mag, sigma_vel, sigma_lag, magnitude_differences, velocity_differences, lag_differences, residual_time_pos, residual_height_pos = RMSD_calc_diff(data, fit_funct) 
    sigma_mag = sigma_mag/fit_funct['stdev_mag']
    sigma_vel = sigma_vel/fit_funct['stdev_vel']
    
    # smooth the condition for the edge cases
    # # delete one the first  np.max(abs(magnitude_differences)) if np.max(abs(velocity_differences))<fit_funct['stdev_vel']
    # if np.max(abs(velocity_differences))<fit_funct['stdev_vel']:
    #     # only if the it is not a isolated peak in the magnitude
    #     magnitude_differences = np.delete(magnitude_differences, np.argmax(abs(magnitude_differences)))

    #     # Compute the sigma values adding a factor that is an order of magnitude smaller denominator to account for the bad fitting function
    #     sigma_mag = np.max(abs(magnitude_differences))/(fit_funct['stdev_mag']+10**(len(str(int(abs(value)))) - 1))
    #     sigma_vel = np.max(abs(velocity_differences))/fit_funct['stdev_vel']

    # elif np.max(abs(magnitude_differences))<fit_funct['stdev_mag']:
    #     velocity_differences = np.delete(velocity_differences, np.argmax(abs(velocity_differences)))

    #     # Compute the sigma values adding a factor that is an order of magnitude smaller denominator to account for the bad fitting function
    #     sigma_mag = np.max(abs(magnitude_differences))/fit_funct['stdev_mag']
    #     sigma_vel = np.max(abs(velocity_differences))/fit_funct['stdev_vel']
    ...
    data_picklefile_pd = {
        ...
        'MAX_mag_at_sigma': [sigma_mag],
        'MAX_vel_at_sigma': [sigma_vel],
...
def PCASim(df_sim_shower, df_obs_shower, OUT_PUT_PATH, PCA_percent=99, N_sim_sel=0, variable_PCA=[], No_var_PCA=['MAX_mag_at_sigma','MAX_vel_at_sigma','v_init_180km','a1_acc_jac','a2_acc_jac','a_acc','b_acc','c_acc','c_mag_init','c_mag_end','a_t0', 'b_t0', 'c_t0'], file_name_obs='', cores_parallel=None, PCA_pairplot=False, esclude_real_solution_from_selection=False):
    '''
...
def PCA_physicalProp_KDE_MODE_PLOT(df_sim, df_obs, df_sel, n_PC_in_PCA, fit_funct, mag_noise_real, len_noise_real, mag_stdev, vel_stdev, fps=32, Metsim_folderfile_json='', file_name_obs='', folder_file_name_real='', output_dir='', save_results_folder_events_plots='', total_distribution=False, save_log=False):
    ...
    for jj in range(len(df_obs)):
        ...
        plot_side_by_side(data_file_real, fig, ax, 'go', file_name_obs[:15]+'\nRMSDmag '+str(round(mag_noise,3))+' RMSDvel '+str(round(vel_noise,3)), residuals_mag_real, residuals_vel_real, residual_time_pos_real, residual_height_pos_real, fit_funct, mag_noise, vel_noise,'Std.dev. realizations')
        ...
        if len(curr_sel)<2:
            print('Check if the event is below sigma')
            ii=0
            Metsim_flag=False
            try:
            ...
            sigma_mag = np.max(abs(residuals_mag))/fit_funct['stdev_mag']
            sigma_vel = np.max(abs(residuals_vel))/fit_funct['stdev_vel']
...
σmag '+str(round(curr_sel.iloc[ii]['MAX_mag_at_sigma'],3))+' σvel '+str(round(curr_sel.iloc[ii]['MAX_vel_at_sigma'],3))+'\n\
        ...
        er.coeff:'+str(round(curr_sel.iloc[ii]['erosion_coeff'],3))+' er.index:'+str(round(curr_sel.iloc[ii]['erosion_mass_index'],2)), residuals_mag, residuals_vel, residual_time_pos, residual_height_pos)
                ...
                plot_side_by_side(data_file, fig, ax, '-','σmag '+str(round(curr_sel.iloc[ii]['MAX_mag_at_sigma'],3))+' σvel '+str(round(curr_sel.iloc[ii]['MAX_vel_at_sigma'],3))+' \n\
        m:'+str('{:.2e}'.format(curr_sel.iloc[ii]['mass'],1))+' F:'+str(round(curr_sel.iloc[ii]['F'],2))+'\n\
        rho:'+str(round(curr_sel.iloc[ii]['rho']))+' sigma:'+str(round(curr_sel.iloc[ii]['sigma'],4))+'\n\
        er.height:'+str(round(curr_sel.iloc[ii]['erosion_height_start'],2))+' er.log:'+str(round(curr_sel.iloc[ii]['erosion_range'],1))+'\n\
        er.coeff:'+str(round(curr_sel.iloc[ii]['erosion_coeff'],3))+' er.index:'+str(round(curr_sel.iloc[ii]['erosion_mass_index'],2)), residuals_mag, residuals_vel, residual_time_pos, residual_height_pos)
            ...
            if curr_sel.iloc[ii]['MAX_mag_at_sigma']<mag_stdev and curr_sel.iloc[ii]['MAX_vel_at_sigma']<vel_stdev:
            ...
            plot_side_by_side(gensim_data_sim, fig, ax, 'r-', 'MODE : σmag '+str(round(sigma_mag,3))+' σvel '+str(round(sigma_vel,3))+'\n\
        ...
        er.coeff:'+str(round(pd_datafram_PCA_sim.iloc[0]['erosion_coeff']*1000000,3))+' er.index:'+str(round(pd_datafram_PCA_sim.iloc[0]['erosion_mass_index'],2)), residuals_mag, residuals_vel, residual_time_pos, residual_height_pos)
            ...
            # print('real noise mag', round(mag_noise_real,3),''+str(SIGMA_ERR)+'sig',round(mag_stdev,3),''+str(SIGMA_ERR*2)+'sig',round(mag_stdev*2,3),'|| MODE noise mag', round(rmsd_mag,3), '\nreal noise len', round(len_noise_real/1000,3),''+str(SIGMA_ERR)+'sig',round(vel_stdev,3),''+str(SIGMA_ERR*2)+'sig',round(vel_stdev*2,3),'|| MODE noise len', round(rmsd_lag,3))
            select_mode_print='No'
            if sigma_mag<mag_stdev and sigma_vel<vel_stdev:
            ...
            if densest_point!='':
                ...
                sigma_mag = np.max(abs(residuals_mag))/fit_funct['stdev_mag']
                sigma_vel = np.max(abs(residuals_vel))/fit_funct['stdev_vel']
                # print('real noise mag', round(mag_noise_real,3),''+str(SIGMA_ERR)+'sig',round(mag_stdev,3),''+str(SIGMA_ERR*2)+'sig',round(mag_stdev*2,3),'|| Dens.point noise mag', round(rmsd_mag,3), '\nreal noise len', round(len_noise_real/1000,3),''+str(SIGMA_ERR)+'sig',round(vel_stdev,3),''+str(SIGMA_ERR*2)+'sig',round(vel_stdev*mag_stdev*2,3),'|| Dens.point noise len', round(rmsd_lag,3))
            
                plot_side_by_side(gensim_data_sim, fig, ax, 'b-', 'Dens.point : σmag '+str(round(sigma_mag,3))+' σvel '+str(round(sigma_vel,3))+'\n\
        m:'+str('{:.2e}'.format(pd_datafram_PCA_sim.iloc[0]['mass'],1))+' F:'+str(round(pd_datafram_PCA_sim.iloc[0]['F'],2))+'\n\
        rho:'+str(round(pd_datafram_PCA_sim.iloc[0]['rho']))+' sigma:'+str(round(pd_datafram_PCA_sim.iloc[0]['sigma']*1000000,4))+'\n\
        er.height:'+str(round(pd_datafram_PCA_sim.iloc[0]['erosion_height_start'],2))+' er.log:'+str(round(pd_datafram_PCA_sim.iloc[0]['erosion_range'],1))+'\n\
        er.coeff:'+str(round(pd_datafram_PCA_sim.iloc[0]['erosion_coeff']*1000000,3))+' er.index:'+str(round(pd_datafram_PCA_sim.iloc[0]['erosion_mass_index'],2)), residuals_mag, residuals_vel, residual_time_pos, residual_height_pos)
                ...
                # pd_datafram_PCA_sim['sigma']=pd_datafram_PCA_sim['sigma']/1000000
                select_kde_print='No'
                if rmsd_mag<mag_stdev and rmsd_lag<vel_stdev:
...
def RMSD_calc_diff_old_WEIG(data_file, fit_funct):
    ...
    sort_idx_data = np.argsort(time_data)
    time_data = time_data[sort_idx_data]
    height_km_data = height_km_data[sort_idx_data]
    abs_mag_data = abs_mag_data[sort_idx_data]
    vel_kms_data = vel_kms_data[sort_idx_data]
    lag_kms_data = lag_kms_data[sort_idx_data]

    sort_idx_fit = np.argsort(time_fit)
    time_fit = time_fit[sort_idx_fit]
    height_km_fit = height_km_fit[sort_idx_fit]
    abs_mag_fit = abs_mag_fit[sort_idx_fit]
    vel_kms_fit = vel_kms_fit[sort_idx_fit]
    lag_kms_fit = lag_kms_fit[sort_idx_fit]
    ...
    valid_fit_indices = (time_fit >= common_time_min) & (time_fit <= common_time_max)
    if not np.any(valid_fit_indices):
        print('No valid fit data in overlapping time range')
        return 9999, 9999, 9999, 9999, 9999, 9999, time_fit[0], height_km_fit[0]

    time_fit = time_fit[valid_fit_indices]
    height_km_fit = height_km_fit[valid_fit_indices]
    abs_mag_fit = abs_mag_fit[valid_fit_indices]
    vel_kms_fit = vel_kms_fit[valid_fit_indices]
    lag_kms_fit = lag_kms_fit[valid_fit_indices]

    # Interpolate data_file onto the time grid of fit_funct
    interp_kind = 'linear'  # You can change this to 'nearest', 'quadratic', etc., if appropriate
    interp_abs_mag_data = interp1d(time_data, abs_mag_data, kind=interp_kind, bounds_error=False, fill_value='extrapolate')
    interp_vel_kms_data = interp1d(time_data, vel_kms_data, kind=interp_kind, bounds_error=False, fill_value='extrapolate')
    interp_lag_kms_data = interp1d(time_data, lag_kms_data, kind=interp_kind, bounds_error=False, fill_value='extrapolate')
    interp_height_km_data = interp1d(time_data, height_km_data, kind=interp_kind, bounds_error=False, fill_value='extrapolate')

    # Get interpolated data at fit_funct times
    abs_mag_data_interp = interp_abs_mag_data(time_fit)
    vel_kms_data_interp = interp_vel_kms_data(time_fit)
    lag_kms_data_interp = interp_lag_kms_data(time_fit)
    height_km_data_interp = interp_height_km_data(time_fit)

    # Calculate differences
    magnitude_differences = abs_mag_data_interp - abs_mag_fit
    velocity_differences = vel_kms_data_interp - vel_kms_fit
    lag_differences = lag_kms_data_interp - lag_kms_fit
    height_differences = height_km_data_interp - height_km_fit

    # Interpolate time_data onto height_km_fit to get time differences at height points
    interp_time_data_height = interp1d(height_km_data, time_data, kind=interp_kind, bounds_error=False, fill_value='extrapolate')
    time_data_at_fit_heights = interp_time_data_height(height_km_fit)
    time_differences_at_heights = time_data_at_fit_heights - time_fit

    # Identify indices where time differences exceed threshold
    time_off_indices = np.abs(time_differences_at_heights) > TIME_THRESHOLD

    # Apply penalties to magnitude differences where time differences exceed threshold
    magnitude_differences[time_off_indices] *= PENALITY_FACTOR

    # Identify indices where height differences exceed threshold
    height_off_indices = np.abs(height_differences) > HEIGHT_THRESHOLD

    # Apply penalties to velocity and lag differences where height differences exceed threshold
    velocity_differences[height_off_indices] *= PENALITY_FACTOR
    lag_differences[height_off_indices] *= PENALITY_FACTOR

    # Now, check if the initial or final heights differ by more than the threshold
    initial_height_data = height_km_data[0]
    initial_height_fit = height_km_fit[0]
    final_height_data = height_km_data[-1]
    final_height_fit = height_km_fit[-1]

    initial_height_diff = np.abs(initial_height_data - initial_height_fit)
    final_height_diff = np.abs(final_height_data - final_height_fit)

    # If initial heights differ by more than threshold, apply penalty to entire velocity and lag differences
    if initial_height_diff > HEIGHT_THRESHOLD:
        velocity_differences *= PENALITY_FACTOR
        lag_differences *= PENALITY_FACTOR

    # Similarly for final heights
    if final_height_diff > HEIGHT_THRESHOLD:
        velocity_differences *= PENALITY_FACTOR
        lag_differences *= PENALITY_FACTOR
    ...
    residual_time_pos = time_fit
    residual_height_pos = height_km_fit
...
def RMSD_calc_diff(data_file, fit_funct):
    ...
    threshold_mag = fit_funct['stdev_mag']
    threshold_vel = fit_funct['stdev_vel']
    threshold_lag = fit_funct['stdev_lag']
    fps = fit_funct['fps']

    # # compute the new lag for lag_kms_data
    # lag_sampled=len_sampled-(vel_sampled[0]*time_sampled+len_sampled[0])
    ...
    interp_lag_kms_fit = interp1d(height_km_fit, lag_kms_fit, kind='linear', bounds_error=False, fill_value='extrapolate')
    # Interpolated fit on data grid
    abs_mag_fit_interp = interp_abs_mag_fit(height_km_data)
    vel_kms_fit_interp = interp_vel_kms_fit(height_km_data)
    lag_kms_fit_interp = interp_lag_kms_fit(height_km_data)
    # Calculate differences
    magnitude_differences = abs_mag_fit_interp - abs_mag_data
    velocity_differences = vel_kms_fit_interp - vel_kms_data
    ...
    # Interpolation on the fit data's height grid
    interp_abs_mag_data = interp1d(height_km_data, abs_mag_data, kind='linear', bounds_error=False, fill_value='extrapolate')
    interp_time_data = interp1d(height_km_data, time_data, kind='linear', bounds_error=False, fill_value='extrapolate')
    # Interpolation on the data fit's height grid
    interp_abs_mag_fit = interp1d(height_km_fit, abs_mag_fit, kind='linear', bounds_error=False, fill_value='extrapolate')
    ...
    abs_mag_data_interp = interp_abs_mag_data(height_range)
    time_data_interp = interp_time_data(height_range)
    abs_mag_fit_interp = interp_abs_mag_fit(height_range)
    time_fit_interp = interp_time_fit(height_range)
    time_fit_interp = abs(time_fit_interp-time_fit_interp[0])
    ...
    # # Recalculate lag_sampled using fitted initial conditions
    # lag_sampled_adj = len_km_data_interp - (vel_kms_fit_interp[0] * time_fit_interp + len_km_fit_interp[0])

    # lag_differences = lag_kms_data_interp - lag_kms_fit_interp
    # # lag_sampled_adj = len_km_data_interp - (vel_kms_fit[0] * time_fit_interp + len_km_fit[0])
    # lag_sampled_adj = len_km_data - (vel_kms_fit[0] * time_data + len_km_fit[0])
    ...
    lag_sampled_adj = len_km_data - (vel_kms_fit_interp[0] * time_data + len_km_fit_interp[0])
    lag_differences = lag_kms_fit_interp - lag_sampled_adj

    # prit the RMSD and the stdev of the differences
    print('RMSD mag:', np.sqrt(np.mean(magnitude_differences**2)), 'stdev mag:', np.std(magnitude_differences))
    print('RMSD lag:', np.sqrt(np.mean(lag_differences**2)), 'stdev lag:', np.std(lag_differences))
    print('RMSD vel:', np.sqrt(np.mean(velocity_differences**2)), 'stdev vel:', np.std(velocity_differences))
    ...
    plt.plot(time_data, lag_sampled_adj, 'k-', label='lag_sampled_adj')
    plt.plot(time_data, lag_kms_fit_interp, 'k.', label='lag_kms_fit_interp')
    ...
    magnitude_differences_stdev = magnitude_differences
    # time_stdev, magnitude_differences_stdev = adaptive_sampling_random_function(x_fine, y_fine, y_threshold, threshold, fine_sampling_rate, coarse_sampling_rate):
    time_stdev, velocity_differences_stdev = adaptive_sampling_random_function(time_data, vel_kms_data, vel_kms_fit_interp, threshold_vel, abs(time_data[0]-time_data[1]), 1/fps)
    time_stdev, lag_differences_stdev = adaptive_sampling_random_function(time_data, lag_sampled_adj, lag_kms_fit_interp, threshold_lag, abs(time_data[0]-time_data[1]), 1/fps)
    ...
    # take the standarad deviation of magnitude_differences
    stdev_mag = np.std(magnitude_differences_stdev)*height_diff_penality_factor
    # take the standarad deviation of velocity_differences
    stdev_vel = np.std(velocity_differences_stdev)*time_diff_penality_factor
    # take the standarad deviation of lag_differences
    stdev_lag = np.std(lag_differences_stdev)*time_diff_penality_factor

    # Handle NaNs in RMSD calculations
    if np.isnan(stdev_mag):
        stdev_mag = 9999
    if np.isnan(stdev_vel):
        stdev_vel = 9999
    if np.isnan(stdev_lag):
        stdev_lag = 9999

    residual_time_pos = time_fit
    residual_height_pos = height_km_fit

    return stdev_mag, stdev_vel, stdev_lag, magnitude_differences, velocity_differences, lag_differences, residual_time_pos, residual_height_pos


def adaptive_sampling_random_function(x_fine, y_fine, y_threshold, threshold, fine_sampling_rate, coarse_sampling_rate):
    ...
    # Plotting the results
    plt.figure(figsize=(10, 5))
    plt.plot(x_dense, y_dense, label="Data (Interpolated)")
    plt.scatter(x_combined, y_combined, color="red", label="New Saple points")
    plt.plot(x_fine, y_threshold, label="Fit", color="black", linestyle="--")
...
def PCA_LightCurveSIGMAPLOT_optimize(df_sel_shower, df_obs_shower, output_dir, fit_funct='', gen_Metsim='', mag_noise_real = 0.1, len_noise_real = 20.0, mag_stdev = 2, vel_stdev = 2, fps=32, file_name_obs='', save_results_folder_events_plots='', run_optimization=True):
    ...
    for ii in range(len(curr_sel)):
        ...
        # print('real noise mag', round(mag_noise_real,3),''+str(SIGMA_ERR)+'sig',round(mag_stdev,3),''+str(SIGMA_ERR*2)+'sig',round(mag_stdev*2,3),'|| Event noise mag', round(rmsd_mag,3), '\nreal noise len', round(len_noise_real/1000,3),''+str(SIGMA_ERR)+'sig',round(vel_stdev,3),''+str(SIGMA_ERR*2)+'sig',round(vel_stdev*mag_stdev*2,3),'|| Event noise len', round(rmsd_lag,3))
        plot_side_by_side(data_file_real, fig, ax, 'go', file_name_obs[:15]+'\nRMSDmag '+str(round(mag_noise,3))+' RMSDvel '+str(round(vel_noise,3)), residuals_mag_real, residuals_vel_real, residual_time_pos_real, residual_height_pos_real, fit_funct, mag_noise, vel_noise, 'Std.dev. realizations')
        plot_side_by_side(fit_funct, fig, ax, 'k--','Fit', np.zeros(len(residual_height_pos_real)), np.zeros(len(residual_time_pos_real)), residual_time_pos_real, residual_height_pos_real)
        
        color_line=next(infinite_color_cycle)

        rmsd_mag = curr_sel.iloc[ii]['MAX_mag_at_sigma']
        rmsd_lag = curr_sel.iloc[ii]['MAX_vel_at_sigma']
...
σmag '+str(round(curr_sel.iloc[ii]['MAX_mag_at_sigma'],3))+' σvel '+str(round(curr_sel.iloc[ii]['MAX_vel_at_sigma'],3))+'\n\
        ...
        if run_optimization:
            ...
            if not os.path.isfile(file_json_save_phys):
                ...
                if curr_sel.iloc[ii]['MAX_mag_at_sigma']<mag_stdev and curr_sel.iloc[ii]['MAX_vel_at_sigma']<vel_stdev:
                    ...
                    pd_selected_low_stdev = curr_sel.iloc[ii].copy()
                    pd_selected_low_stdev['solution_id']=file_json_save_phys

                    pd_datafram_PCA_selected_optimized = pd.concat([pd_datafram_PCA_selected_optimized, pd_selected_low_stdev], axis=0)
                    ...
                    # make more space
                    plt.tight_layout()
                    plt.savefig(output_dir+os.sep+save_results_folder_events_plots+os.sep+file_name_title[:23]+'_stdevmag'+str(round(rmsd_mag,2))+'_stdevlen'+str(round(rmsd_lag,2))+'_Heigh_MagVelCoef.png')
                    shutil.copy(output_dir_optimized+os.sep+file_name_obs+'_sim_fit_fitted.json', file_json_save_results)

                    plt.savefig(output_dir+os.sep+SAVE_SELECTION_FOLDER+os.sep+file_name_title[:23]+'_stdevmag'+str(round(rmsd_mag,2))+'_stdevlen'+str(round(rmsd_lag,2))+'_Heigh_MagVelCoef.png')

                    # close the plot
                    plt.close()
                    continue

                elif curr_sel.iloc[ii]['MAX_mag_at_sigma']<mag_stdev*1.1 and curr_sel.iloc[ii]['MAX_vel_at_sigma']<vel_stdev*1.1: # 95CI 1.96 z-score
                ...
                elif curr_sel.iloc[ii]['MAX_mag_at_sigma']<mag_stdev*2 and curr_sel.iloc[ii]['MAX_vel_at_sigma']<vel_stdev*2: # 99.99CI 1.96*2 z-score
            ...
            # print('real noise mag', round(mag_noise_real,3),''+str(SIGMA_ERR)+'sig',round(mag_noise_real,3),''+str(SIGMA_ERR*2)+'sig',round(mag_stdev*2,3),'|| Event noise mag', round(rmsd_mag,3), '\nreal noise len', round(len_noise_real/1000,3),''+str(SIGMA_ERR)+'sig',round(vel_stdev,3),''+str(SIGMA_ERR*2)+'sig',round(vel_stdev*2,3),'|| Event noise len', round(rmsd_lag,3))

            if Metsim_flag:
                
                plot_side_by_side(gensim_data_optimized, fig, ax, 'k--', 'Optimized MetSim σmag '+str(round(rmsd_mag,3))+' σvel '+str(round(rmsd_lag,3))+'\n\
    ...
    er.coeff:'+str(round(pd_datafram_PCA_sim_optimized.iloc[0]['erosion_coeff']*1000000,3))+' er.index:'+str(round(pd_datafram_PCA_sim_optimized.iloc[0]['erosion_mass_index'],2)), residuals_mag, residuals_vel, residual_time_pos, residual_height_pos)
                ...
                plot_side_by_side(gensim_data_optimized, fig, ax, '--', 'Optimized σmag '+str(round(rmsd_mag,3))+' σvel '+str(round(rmsd_lag,3))+'\n\
    m:'+str('{:.2e}'.format(pd_datafram_PCA_sim_optimized.iloc[0]['mass'],1))+' F:'+str(round(pd_datafram_PCA_sim_optimized.iloc[0]['F'],2))+'\n\
    rho:'+str(round(pd_datafram_PCA_sim_optimized.iloc[0]['rho']))+' sigma:'+str(round(pd_datafram_PCA_sim_optimized.iloc[0]['sigma']*1000000,4))+'\n\
    er.height:'+str(round(pd_datafram_PCA_sim_optimized.iloc[0]['erosion_height_start'],2))+' er.log:'+str(round(pd_datafram_PCA_sim_optimized.iloc[0]['erosion_range'],1))+'\n\
    er.coeff:'+str(round(pd_datafram_PCA_sim_optimized.iloc[0]['erosion_coeff']*1000000,3))+' er.index:'+str(round(pd_datafram_PCA_sim_optimized.iloc[0]['erosion_mass_index'],2)), residuals_mag, residuals_vel, residual_time_pos, residual_height_pos)
        ...
        if rmsd_mag<mag_stdev and rmsd_lag<vel_stdev:
            ...
            if run_optimization:
                print('below noise, OPTIMIZED and SAVED')
            ...
            if run_optimization:
                ...
                # remove curr_sel.iloc[[ii]].drop(columns=['MAX_mag_at_sigma', 'MAX_vel_at_sigma', 'solution_id_dist', 'distance_meteor', 'distance_mean']) rmsd_mag	rmsd_len solution_id_dist	distance_meteor	distance_mean
                pd_datafram_PCA_selected_optimized = pd.concat([pd_datafram_PCA_selected_optimized, pd_datafram_PCA_sim], axis=0)

        else:
            print('above the noise, NOT SAVED')
            if run_optimization:
    ...
    # # save df_sel_shower_real to disk add the σ
...
def PCA_LightCurveCoefPLOT(df_sel_shower_real, df_obs_shower, output_dir, fit_funct, gensim_data_obs='', mag_noise_real= 0.1, len_noise_real = 20.0, fps=32, file_name_obs='', trajectory_Metsim_file='', output_folder_of_csv=''):
    ...
    for ii in range(len(curr_sel)):
        ...
        if not os.path.isfile(namefile_sel):
            ...
            if namefile_sel.endswith('.pickle'):
                ...
                if 'ht_sampled' in data:
                    ...
                    # split trajectory_Metsim_file directory and file and keep the file name
        ...
        if ii==0:
            ...
            ax[1].plot(obs_time, vel_kms,label=file_name_obs[:15]+'\nsigmamag '+str(round(mag_noise_real,3))+' sigmavel '+str(round(len_noise_real/1000,3)))
...
σmag '+str(round(curr_sel.iloc[ii]['MAX_mag_at_sigma'],3))+' σvel '+str(round(curr_sel.iloc[ii]['MAX_vel_at_sigma'],3))+'\n\
    ...
    er.coeff:'+str(round(curr_sel.iloc[ii]['erosion_coeff'],3))+' er.index:'+str(round(curr_sel.iloc[ii]['erosion_mass_index'],2)))                                                                         
                ...
                ax[1].plot(obs_time, vel_kms, color=line_color ,label='σmag '+str(round(curr_sel.iloc[ii]['MAX_mag_at_sigma'],3))+' σvel '+str(round(curr_sel.iloc[ii]['MAX_vel_at_sigma'],3))+'\n\
    m:'+str('{:.2e}'.format(curr_sel.iloc[ii]['mass'],1))+' F:'+str(round(curr_sel.iloc[ii]['F'],2))+'\n\
    rho:'+str(round(curr_sel.iloc[ii]['rho']))+' sigma:'+str(round(curr_sel.iloc[ii]['sigma'],4))+'\n\
    er.height:'+str(round(curr_sel.iloc[ii]['erosion_height_start'],2))+' er.log:'+str(round(curr_sel.iloc[ii]['erosion_range'],1))+'\n\
    er.coeff:'+str(round(curr_sel.iloc[ii]['erosion_coeff'],3))+' er.index:'+str(round(curr_sel.iloc[ii]['erosion_mass_index'],2)))
        ...
        # df_sel_shower_real.loc[df_sel_shower_real['solution_id']==curr_sel.iloc[ii]['solution_id'],'MAX_mag_at_sigma'] = rmsd_mag
        # # df_sel_shower_real.loc[df_sel_shower_real['solution_id']==curr_sel.iloc[ii]['solution_id'],'rmsd_vel'] = rmsd_vel
        # df_sel_shower_real.loc[df_sel_shower_real['solution_id']==curr_sel.iloc[ii]['solution_id'],'MAX_vel_at_sigma'] = rmsd_lag
...
if __name__ == "__main__":
    ...
    arg_parser.add_argument('--max_sigma_mag', metavar='MAX_SIGMA_MAG', type=float, default=1, \
        help="Maximum value of sigma allowed for absolute Magnitude.")
    
    arg_parser.add_argument('--max_sigma_vel', metavar='MAX_SIGMA_VEL', type=float, default=1, \
        help="Maximum value of sigma allowed for absolute Velocity.")
    ...
    arg_parser.add_argument('--NoPCA', metavar='NOPCA', type=str, default=['MAX_mag_at_sigma','MAX_vel_at_sigma','v_init_180km','a1_acc_jac','a2_acc_jac','a_acc','b_acc','c_acc','c_mag_init','c_mag_end','a_t0', 'b_t0', 'c_t0'], \
    ...
    for trajectory_file, file_name, input_folder, output_folder, trajectory_Metsim_file in input_folder_file:
        ...
        if trajectory_file.endswith('.csv'):
            ...
            pd_dataframe_PCA_obs_real = array_to_pd_dataframe_PCA(gensim_data_obs, fit_funct)
            ...
            # add to all the rows the rmsd_mag_obs, rmsd_lag_obs, SIGMA cannot work for noisy simulations because of interp
            # pd_dataframe_PCA_obs_real['MAX_mag_at_sigma'] = rmsd_pol_mag
            # pd_dataframe_PCA_obs_real['MAX_vel_at_sigma'] = rmsd_t0_lag/1000
        ...
        # Not important only need the sigma_mag and sigma_vel to be present (considered as all within 1 sigma)
        pd_dataframe_PCA_obs_real['MAX_mag_at_sigma'] = cml_args.max_sigma_mag
        pd_dataframe_PCA_obs_real['MAX_vel_at_sigma'] = cml_args.max_sigma_vel
        ...
        ### ORDER BASE ON BOTH SIGMA ###

        # deep copy pd_datafram_PCA_sim
        pd_datafram_check_sim = pd_datafram_PCA_sim.copy(deep=True)

        # Normalize the columns to bring them to the same scale
        pd_datafram_check_sim['MAX_mag_at_sigma_norm'] = pd_datafram_check_sim['MAX_mag_at_sigma'] / pd_datafram_check_sim['MAX_mag_at_sigma'].max()
        pd_datafram_check_sim['MAX_vel_at_sigma_norm'] = pd_datafram_check_sim['MAX_vel_at_sigma'] / pd_datafram_check_sim['MAX_vel_at_sigma'].max()

        # Compute the combined metric (e.g., sum of absolute normalized values)
        pd_datafram_check_sim['combined_metric'] = abs(pd_datafram_check_sim['MAX_mag_at_sigma_norm']) + abs(pd_datafram_check_sim['MAX_vel_at_sigma_norm'])

        # Sort the DataFrame based on the combined metric
        pd_datafram_check_sim = pd_datafram_check_sim.sort_values(by='combined_metric')

        # Reset index if needed
        pd_datafram_check_sim = pd_datafram_check_sim.reset_index(drop=True)

        # check if the following values are also smaller than cml_args.max_sigma_mag and cml_args.max_sigma_vel and save the index
        for i in range(0, len(pd_datafram_check_sim)):
            if pd_datafram_check_sim['MAX_mag_at_sigma'].iloc[i] > cml_args.max_sigma_mag or pd_datafram_check_sim['MAX_vel_at_sigma'].iloc[i] > cml_args.max_sigma_vel:
                index = i-1
                break
        
        # duplicate pd_datafram_check_sim
        pd_datafram_check_below_stdev = pd_datafram_check_sim.copy(deep=True)

        # take the head of the dataframe with the index
        pd_datafram_check_below_stdev = pd_datafram_check_below_stdev.head(index+1)

        # Drop the auxiliary columns if they are no longer needed
        pd_datafram_check_below_stdev = pd_datafram_check_below_stdev.drop(columns=['MAX_mag_at_sigma_norm', 'MAX_vel_at_sigma_norm', 'combined_metric'])
        
        ##############################

        # check if a file with the name "log"+n_PC_in_PCA+"_"+str(len(df_sel))+"ev.txt" already exist print('processing file:',file_name)
        if os.path.exists(output_folder+os.sep+"log_"+file_name[:15]+"_set_up.txt"):
            # remove the file
            os.remove(output_folder+os.sep+"log_"+file_name[:15]+"_set_up.txt")
        sys.stdout = Logger(output_folder,"log_"+file_name[:15]+"_set_up.txt") # _30var_99%_13PC

        print('Run MetSim file:',trajectory_Metsim_file)

        # print len of the csv file
        print('Number of simulations:',len(pd_datafram_PCA_sim))

        # print the time of the observation
        print(cml_args.max_sigma_mag,'sigmaMAG and',cml_args.max_sigma_vel,'sigmaVEL')
        # print the time of the observation
        print('standard deviation 1sigmaMag:',np.round(rmsd_pol_mag,3),'[mag]')
        print('standard deviation 1sigmaLen:',np.round(rmsd_t0_lag/1000,3),'[km]')
        print('standard deviation 1sigmaVel:',np.round(rmsd_t0_lag/1000*np.sqrt(2)/(1/cml_args.fps),3),'[km/s]')


        # check if in pd_datafram_PCA_sim there are any where rmsd_mag and rmsd_len are below SIGMAmag and sigmaVEL
        print('Number of simulations below sigma:', len(pd_datafram_check_below_stdev))
        print('Threshold mag', cml_args.max_sigma_mag,'sigma vel', cml_args.max_sigma_vel,'sigma')
        # print the first cml_args.max_sigma_mag and cml_args.max_sigma_vel
        print('Best simulation below ', pd_datafram_check_below_stdev['MAX_mag_at_sigma'].iloc[0],'sigmaMAG and', pd_datafram_check_below_stdev['MAX_vel_at_sigma'].iloc[0],'sigmaVEL')

        if pd_datafram_check_below_stdev['MAX_mag_at_sigma'].iloc[0] > cml_args.max_sigma_mag and pd_datafram_check_below_stdev['MAX_vel_at_sigma'].iloc[0] > cml_args.max_sigma_vel:
            print('The best simulation is above',cml_args.max_sigma_mag,'sigmaMAG and',cml_args.max_sigma_vel,'sigmaVEL')
            print('The best simulation is not good enough')
        ...
        pd_datafram_PCA_selected_lowSIGMA = pd.DataFrame()
        ...
        flag_MODE_KDE_below_stdev = False
        pca_N_comp = 0
        ...
        if cml_args.use_PCA:
            ...
            print('PLOT: best 9 simulations selected and add the sigma value to csv selected')
            # plot of the best 9 selected simulations and add the sigma value to csv selected
            PCA_LightCurveCoefPLOT(pd_datafram_PCA_selected_before_knee_NO_repetition, pd_dataframe_PCA_obs_real, output_folder, fit_funct, gensim_data_obs, rmsd_pol_mag, rmsd_t0_lag, fps, file_name, trajectory_Metsim_file)
            ...
            # concatenate the two dataframes
            pd_datafram_PCA_selected_lowSIGMA = pd.concat([pd_datafram_PCA_selected_mode_min_KDE_TOT, pd_datafram_PCA_selected_mode_min_KDE])
            # reset index
            pd_datafram_PCA_selected_lowSIGMA.reset_index(drop=True, inplace=True)

            pd_datafram_PCA_selected_lowSIGMA['type'] = 'Simulation_sel'
        
            # # save df_sel_shower_real to disk add the SIGMA
            # pd_datafram_PCA_selected_lowSIGMA.to_csv(output_folder+os.sep+save_results_folder+os.sep+file_name+'_sim_sel_results.csv', index=False)
            ...
            flag_MODE_KDE_below_stdev = True
            # print(pd_datafram_PCA_selected_lowSIGMA)
            # split in directory and filename
            filename_list = []
            # print(pd_datafram_PCA_selected_lowSIGMA['solution_id'].values)
            if 'solution_id' in pd_datafram_PCA_selected_lowSIGMA.columns:
                # check if in pd_datafram_PCA_selected_lowSIGMA there is any json file that is not in the selected simulations
                for solution_id in pd_datafram_PCA_selected_lowSIGMA['solution_id'].values:
                ...
                for json_file in json_files:
                    ...
                    if json_file not in filename_list:
                        ...
                        # Add the simulation results to pd_datafram_PCA_selected_lowSIGMA
                        pd_datafram_PCA_selected_lowSIGMA = pd.concat([pd_datafram_PCA_selected_lowSIGMA, pd_datafram_PCA_sim_resulsts])
                pd_datafram_PCA_selected_lowSIGMA['type'] = 'Simulation_sel'
                pd_datafram_PCA_selected_lowSIGMA.reset_index(drop=True, inplace=True)
            else:
        ...
        ######################## sigma CHECK and OPTIMIZATION ###############################

        print('--- sigma CHECK ---')

        # add a column with solution_id_dist to 9999
        pd_datafram_check_below_stdev['solution_id_dist'] = pd_datafram_check_below_stdev['solution_id'].iloc[0]
        # set type equal 'Simulation_sel'
        pd_datafram_check_below_stdev['type'] = 'Simulation_sel'
        # add column distance_meteor=9999 and distance_mean=9999 and num_duplicates=0
        pd_datafram_check_below_stdev['distance_meteor'] = 9999
        pd_datafram_check_below_stdev['distance_mean'] = 9999
        pd_datafram_check_below_stdev['num_duplicates'] = 0

        if cml_args.use_PCA:
            # delete any rows from pd_datafram_check_below_stdev that has the same ['solution_id'] as pd_datafram_PCA_selected_before_knee_NO_repetition['solution_id']
            pd_datafram_check_below_stdev = pd_datafram_check_below_stdev[~pd_datafram_check_below_stdev['solution_id'].isin(pd_datafram_PCA_selected_before_knee_NO_repetition['solution_id'])]

        # reset index
        pd_datafram_check_below_stdev.reset_index(drop=True, inplace=True)
            
        # concatenate the two dataframes
        pd_datafram_check_below_stdev = pd.concat([pd_datafram_check_below_stdev, pd_datafram_PCA_selected_before_knee_NO_repetition])
        pd_datafram_check_below_stdev.to_csv(output_folder+os.sep+SAVE_SELECTION_FOLDER+os.sep+file_name+'_sim_sel_to_optimize.csv', index=False)


        if pd_datafram_check_below_stdev.empty and flag_MODE_KDE_below_stdev==False:
            print()
            print('ACTIVATE OPTIMIZATION for the first 5')
            print()
            cml_args.optimize=True
            cml_args.number_optimized=5
            # take the head of the dataframe with the index
            pd_datafram_check_below_stdev = pd_datafram_check_sim.head(5)
        ...
        if cml_args.number_optimized > len(pd_datafram_check_below_stdev):
            cml_args.number_optimized = 0

        # # plot the values and find the SIGMA of each of them
        if cml_args.number_optimized == 0:
            input_list_obs = [[pd_datafram_check_below_stdev.iloc[[ii]].reset_index(drop=True), pd_dataframe_PCA_obs_real, output_folder, fit_funct, gensim_data_Metsim, rmsd_pol_mag, rmsd_t0_lag, cml_args.max_sigma_mag, cml_args.max_sigma_vel, fps, file_name, save_results_folder_events_plots, cml_args.optimize] for ii in range(len(pd_datafram_check_below_stdev))]
            results_list = domainParallelizer(input_list_obs, PCA_LightCurveSIGMAPLOT_optimize, cores=cml_args.cores)
            ...
            input_list_obs = [[pd_datafram_check_below_stdev.iloc[[ii]].reset_index(drop=True), pd_dataframe_PCA_obs_real, output_folder, fit_funct, gensim_data_Metsim, rmsd_pol_mag, rmsd_t0_lag, cml_args.max_sigma_mag, cml_args.max_sigma_vel, fps, file_name, save_results_folder_events_plots, True] for ii in range(cml_args.number_optimized)]
            results_list = domainParallelizer(input_list_obs, PCA_LightCurveSIGMAPLOT_optimize, cores=cml_args.cores)
            pd_datafram_PCA_selected_optimized = pd.concat(results_list)
            if cml_args.number_optimized != len(pd_datafram_check_below_stdev):
                # repeat for the rest of the selected events but no Optimization
                input_list_obs = [[pd_datafram_check_below_stdev.iloc[[ii]].reset_index(drop=True), pd_dataframe_PCA_obs_real, output_folder, fit_funct, gensim_data_Metsim, rmsd_pol_mag, rmsd_t0_lag, cml_args.max_sigma_mag, cml_args.max_sigma_vel, fps, file_name, save_results_folder_events_plots, False] for ii in range(cml_args.number_optimized, len(pd_datafram_check_below_stdev))]
                results_list = domainParallelizer(input_list_obs, PCA_LightCurveSIGMAPLOT_optimize, cores=cml_args.cores)
                pd_datafram_PCA_selected_NO_optimizad = pd.concat(results_list)
                pd_datafram_PCA_selected_lowSIGMA = pd.concat([pd_datafram_PCA_selected_optimized, pd_datafram_PCA_selected_NO_optimizad])

        
        # concatenate the two dataframes
        pd_datafram_PCA_selected_lowSIGMA = pd.concat([pd_datafram_PCA_selected_optimized, pd_datafram_PCA_selected_lowSIGMA])

        # check if trajectory_Metsim_file is among the pd_datafram_check_below_stdev
        if flag_manual_metsim:
            print('Check the manual reduction')
            # check also the manual reduction
            pd_datafram_PCA_selected_optimized_Metsim = PCA_LightCurveSIGMAPLOT_optimize(pd_datafram_PCA_sim_Metsim, pd_dataframe_PCA_obs_real, output_folder, fit_funct, gensim_data_Metsim, rmsd_pol_mag, rmsd_t0_lag, cml_args.max_sigma_mag, cml_args.max_sigma_vel, fps, file_name, save_results_folder_events_plots, False) # file_name, trajectory_Metsim_file, 
            
        # concatenate the two dataframes
        pd_datafram_PCA_selected_lowSIGMA = pd.concat([pd_datafram_PCA_selected_optimized_Metsim, pd_datafram_PCA_selected_lowSIGMA])
        ...
        # check if any json_files_results is in pd_datafram_PCA_selected_lowSIGMA['solution_id'].values
        if 'solution_id' in pd_datafram_PCA_selected_lowSIGMA.columns:
            for json_file in json_files_results:
                if json_file not in pd_datafram_PCA_selected_lowSIGMA['solution_id'].values:
                    # print that is found a json file that is not in the selected simulations
                    ...
                    rmsd_mag, rmsd_vel, rmsd_lag, _, _, _, _, _ = RMSD_calc_diff(data_file, fit_funct)
                    # Add the simulation results that have a rmsd_mag and rmsd_len that is below SIGMA to pd_datafram_PCA_selected_lowSIGMA
                    if rmsd_mag <= cml_args.max_sigma_mag and rmsd_lag <= cml_args.max_sigma_vel:
            ...
            print('No solutions below the sigma for the selected simulations')
        ...
        print(cml_args.min_nresults,'simulatd to found')
        while cml_args.min_nresults > result_number:
            ...
            if 'mass' in pd_datafram_PCA_selected_lowSIGMA.columns:                  
                # Drop duplicate rows based on the specified columns
                pd_datafram_PCA_selected_lowSIGMA = pd_datafram_PCA_selected_lowSIGMA.drop_duplicates(subset=[
            ...
            pd_results = pd.concat([pd_results, pd_datafram_PCA_selected_lowSIGMA])
            ...
            if 'solution_id' in pd_results.columns:
                ...
                print('PLOT: best 9 results and add the sigma value to csv selected')
                ...
                # print('FAIL: Not found any result below magsigma',rmsd_pol_mag,'and lensigma',rmsd_t0_lag/1000)
                print('FAIL: Not found any result below MAG',cml_args.max_sigma_mag,'sigma and VEL',cml_args.max_sigma_vel,'sigma')
                flag_fail = True
                break
            ...
            if cml_args.min_nresults <= result_number:
                ...
                if ii_repeat==cml_args.ntry:
                    ...
                    print('STOP: No new simulation below MAG',cml_args.max_sigma_mag,'sigma and VEL',cml_args.max_sigma_vel,'sigma')
                ...
                input_list_obs = [[pd_datafram_NEWsim_good.iloc[[ii]].reset_index(drop=True), pd_dataframe_PCA_obs_real, output_folder, fit_funct, gensim_data_Metsim, rmsd_pol_mag, rmsd_t0_lag, cml_args.max_sigma_mag, cml_args.max_sigma_vel, fps, file_name, save_results_folder_events_plots, False] for ii in range(len(pd_datafram_NEWsim_good))]
                results_list = domainParallelizer(input_list_obs, PCA_LightCurveSIGMAPLOT_optimize, cores=cml_args.cores)

                # base on the one selected
                pd_datafram_PCA_selected_lowSIGMA = pd.concat(results_list)
        ...
        if flag_fail == False:
            ...
            print('FAIL: Not found enough result below MAG',cml_args.max_sigma_mag,'sigma and VEL',cml_args.max_sigma_vel,'sigma')
