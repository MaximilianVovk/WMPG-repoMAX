...
only_select_meteors_from='20210805_083806A'
...
# to plot all
df_obs_real = pd.read_csv(os.getcwd()+r'/PER_and_dist.csv')
for ii_met in range(len(df_obs_real)):
  # change the name of only_select_meteors_from
  only_select_meteors_from=df_obs_real.iloc[ii_met]['solution_id']
  ...
  if len(df_obs_shower) == 1 and n_confront_obs>0:
      ...
      if os.path.exists(os.getcwd()+r'/Manual Reduction/'+namefile_manual):
          ...
          # traj = wmpl.Utils.Pickling.loadPickle(f, namefile+".pylig.pickle")
          traj = wmpl.Utils.Pickling.loadPickle("/home/mvovk/PCA/PER_pk/", namefile[:-1]+"_trajectory.pickle")
          for obs in traj.observations:
              ...
              if with_legend:
                  # put it in the first subplot
                  ax[0].plot( obs.absolute_magnitudes,height_km, label=obs.station_id+' Manual Reduct',marker='x',linestyle='--')
  ...
  plt.savefig('images_events/'+only_select_meteors_from+'Heigh_MagVel.png')
  ...
  plt.savefig('images_events/'+only_select_meteors_from+'Time_MagVel.png')
