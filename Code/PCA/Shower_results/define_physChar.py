...
# Usage
directory = r'C:\Users\maxiv\Desktop\old\RunTest_sol'  # Change this to the directory you want to process
shower = 'PER'
    ...
    for root, _, files in os.walk(directory):
        for file in files:
            if file.endswith('_sim_sel_results.csv'):
                ...
                # chek if there is the solution_id column
                if 'solution_id' in df.columns:
                    print('SUCCES: Good data found in', file_path)
                    df['meteor'] = meteor_value
                    selected_results.append(df)
                else:
                    print('FAIL: No data found in', file_path)
                    continue
...
curr_df_sim_sel['weight_group'] = 1 / curr_df_sim_sel['num_group']
