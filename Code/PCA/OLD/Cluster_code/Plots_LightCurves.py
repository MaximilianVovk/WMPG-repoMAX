...
only_select_meteors_from='erosion_sim_v60.98_m1.07e-03g_rho0368_z23.2_abl0.027_eh104.0_er0.181_s1.85.json'
...
for current_shower in Shower:
    ...
    # if only_select_meteors_from is not epty select the shower with the solution_id 
...
for current_shower in Shower:
    ...
    for ii in range(len(curr_obs)):
        ...
        traj = wmpl.Utils.Pickling.loadPickle("M:/emccd/pylig/trajectory/"+folder+"/", namefile+".pylig.pickle") # M:\emccd\pylig\trajectory\20180805\20180805_051718A
...
if len(df_obs_shower) == 1 and n_confront_obs>0:
    ...
    if os.path.exists(os.getcwd()+r'\Manual Reduction/'+namefile_manual):
        ...
        traj = wmpl.Utils.Pickling.loadPickle("C:/Users/maxiv/Documents/UWO/Western Meteor Physics Group/Conferences/20230618 - ACM/Code_use/Manual Reduction/"+namefile_manual+"/", namefile_manual+"_trajectory.pickle")
        for obs in traj.observations:

            height_km=[x/1000 for x in obs.model_ht]
            
            vel_kms=[x/1000 for x in obs.velocities[1:]]
            height_km2=[x/1000 for x in obs.model_ht[1:]]
            
            if with_legend:
                # put it in the first subplot
                ax[0].plot( obs.absolute_magnitudes,height_km, label=obs.station_id+' Manual Reduct')
                ax[1].plot( vel_kms,height_km2, label=obs.station_id+' Manual Reduct')
            else:
                ax[0].plot( obs.absolute_magnitudes,height_km, label=obs.station_id+' Manual Reduct',color='red')
                ax[1].plot( vel_kms,height_km2, label=obs.station_id+' Manual Reduct',color='red')
...
for ii in range(len(curr_sel)):
    ...
    if not os.path.isfile(namefile_sel):
        ...
        if with_legend:
            # put it in the first subplot
            # and make it in points with x connected with a line
            ax[0].plot(data['mag_sampled'],height_km,label='sel_'+current_shower+'('+str(index_sel)+') MEANdist:'+str(round(curr_sel.iloc[ii]['distance'],2)),marker='x',linestyle='--')
        else:
...
for ii in range(len(curr_sel)):
    ...
    if os.path.isfile(namefile_sel):
        ...
        if with_legend:
            ...
            # plt.plot(vel_sim,ht_sim,label='sel_'+current_shower+'('+str(index_sel)+') PCAdist:'+str(round(curr_sel.iloc[ii]['distance'],2)))
    ...
    er.height:'+str(round(curr_sel.iloc[ii]['erosion_height_start']))+'\n\
    er.coeff:'+str(round(curr_sel.iloc[ii]['erosion_coeff'],3))+'\n\
    er.index:'+str(round(curr_sel.iloc[ii]['erosion_mass_index'],2)))
            ...
            # plt.plot(vel_sim,ht_sim,label='sel_'+current_shower+'('+str(index_sel)+') PCAdist:'+str(round(curr_sel.iloc[ii]['distance'],2)),color='coral')
            ax[1].plot(height_km,vel_kms,label='sel_'+current_shower+'('+str(index_sel)+') MEANdist:'+str(round(curr_sel.iloc[ii]['distance'],2)),color='coral')
...
for current_shower in Shower:
    ...
    for ii in range(len(curr_obs)):
        ...
        traj = wmpl.Utils.Pickling.loadPickle("M:/emccd/pylig/trajectory/"+folder+"/", namefile+".pylig.pickle")
        mag_arr=[]
        time_base=[]
        vel_pickl=[]
        time_pickl=[]
