...
exports a reusable batch pickle, combined plot, and flat CSV. An optional
synthetic Mars-speed sweep reruns each fitted physical meteoroid over a selected
velocity grid, remaps erosion/fragmentation triggers at equal dynamic pressure,
and uses those additional simulations to constrain the detection surface. Folder sweeps can run concurrently across all scheduler-allocated CPU cores.
"""
...
def _compact_lightcurve_overlay_data(sim_result: SimulationResults, max_points: int = 220) -> dict[str, list[float]]:
    """Return a compact abs-mag/height profile for later overlay plotting."""
    ...
    n = min(len(abs_mag), len(height_km))
    abs_mag = abs_mag[:n]
    height_km = height_km[:n]
    good = np.isfinite(abs_mag) & np.isfinite(height_km)
    abs_mag = abs_mag[good]
    height_km = height_km[good]

    if abs_mag.size == 0:
        return {"abs_magnitude": [], "height_km": []}

    if abs_mag.size > int(max_points):
        sample_idx = np.linspace(0, abs_mag.size - 1, int(max_points))
        sample_idx = np.unique(np.round(sample_idx).astype(int))
        abs_mag = abs_mag[sample_idx]
        height_km = height_km[sample_idx]

    return {
        "abs_magnitude": abs_mag.tolist(),
        "height_km": height_km.tolist(),
    }
...
def plot_lightcurve_earth_vs_mars_dyn_pressure(
    ...
    output_dir: str | Path,
    base_name: str,
    synthetic_curve_records: list[dict[str, Any]] | None = None,
) -> str:
    """
    ...
    synthetic_curve_records = list(synthetic_curve_records or [])
    synthetic_speed_values = [
        _finite_float(record.get("synthetic_speed_kms"))
        for record in synthetic_curve_records
        if isinstance(record, dict) and record.get("lightcurve_overlay")
    ]
    synthetic_speed_values = [value for value in synthetic_speed_values if np.isfinite(value)]
    synth_norm = None
    synth_cmap = None
    ...
    if synthetic_speed_values:
        ...
        for record in synthetic_curve_records:
            ...
            speed_value = _finite_float(record.get("synthetic_speed_kms"))
            if synth_norm is None or not np.isfinite(speed_value):
                color = "0.55"
            else:
                color = synth_cmap(synth_norm(speed_value))

            detected = bool(record.get(
                "is_camera_detected", record.get("is_physically_detected", False)
            ))
...
def apply_apparent_detection_and_camera_sampling(
    ...
    minimum_detected_frames: int = 10,
) -> dict:
    """
    ...
    Individual visible frames are controlled by::

        m_app(t) <= limiting_app_mag

    A meteor is classified as detected by the camera only when at least
    ``minimum_detected_frames`` sampled frames satisfy that condition.
    ...
    minimum_detected_frames = int(minimum_detected_frames)
    if minimum_detected_frames < 1:
        raise ValueError(
            f"minimum_detected_frames must be at least 1, got {minimum_detected_frames}."
        )

    # Raw photometric visibility is kept separate from the operational camera
    # detection, which additionally requires a minimum number of sampled frames.
    ...
    meets_minimum_frame_requirement = bool(
        physical_detected_frame_count >= minimum_detected_frames
    )
    camera_detected = bool(
        physically_detectable and meets_minimum_frame_requirement
    )
    if camera_detected:
        detection_status = "camera detected"
    elif physically_detectable:
        detection_status = "visible but insufficient frames"
    else:
        detection_status = "not visible above camera limit"
    ...
    print(
        ...
        f"frames above LM: {physical_detected_frame_count}."
    )
    print(
        f"Camera detection requirement: >= {minimum_detected_frames} frames above LM; "
        f"result: {'DETECTED' if camera_detected else 'NOT DETECTED'}."
    )
    ...
    # Backward-compatible raw visibility field. Camera detection is the stricter
    # classification used by all batch plots and likelihood fitting.
    out["is_physically_detectable"] = physically_detectable
    out["is_photometrically_visible"] = physically_detectable
    out["minimum_detected_frames"] = minimum_detected_frames
    out["meets_minimum_frame_requirement"] = meets_minimum_frame_requirement
    out["is_camera_detected"] = camera_detected
    out["detection_status"] = detection_status
    out["detection_criteria"] = (
        f"at least {minimum_detected_frames} sampled frames with "
        f"m_app <= {physical_threshold:.2f}"
    )
...
def save_detection_summary(geometry: dict, out_path: str | Path) -> str:
    ...
    summary = {
        ...
        "physical_camera_detection": {
            "is_detectable": geometry.get(
                "is_camera_detected", geometry.get("is_physically_detectable")
            ),
            "is_photometrically_visible": geometry.get(
                "is_photometrically_visible", geometry.get("is_physically_detectable")
            ),
            "status": geometry.get("detection_status"),
            "criterion": geometry.get("detection_criteria"),
            "minimum_detected_frames": geometry.get("minimum_detected_frames"),
            "meets_minimum_frame_requirement": geometry.get(
                "meets_minimum_frame_requirement"
            ),
...
def plot_mars_detected_3d_view(
    ...
    detectable = bool(geometry.get(
        "is_camera_detected", geometry.get("is_physically_detectable", False)
    ))
    minimum_frames = int(geometry.get("minimum_detected_frames", 1) or 1)
    zc_deg = None if zenith_angle_rad is None else np.degrees(float(zenith_angle_rad))
    label_lines = [
        f'Camera detection: {"DETECTED" if detectable else "NOT DETECTED"}',
        f'Minimum detected frames: {minimum_frames}',
        ...
        f'Frames above LM: {geometry.get("physical_detected_frame_count", 0)}',
    ]
...
def _normalise_minimum_detected_frames(value: Any, default: int = 10) -> int:
    """Return a valid operational minimum-frame requirement."""
    try:
        result = int(value)
    except (TypeError, ValueError):
        result = int(default)
    if result < 1:
        raise ValueError(f"minimum detected frames must be at least 1, got {result}.")
    return result


def apply_minimum_frame_detection_to_record(
    record: dict[str, Any],
    minimum_detected_frames: int,
) -> dict[str, Any]:
    """Reclassify current or cached records without rerunning MetSim."""
    minimum_frames = _normalise_minimum_detected_frames(
        minimum_detected_frames
    )
    frames = int(record.get("detected_frame_count", 0) or 0)
    peak_mag = _finite_float(record.get("peak_apparent_magnitude"))
    limiting_mag = _finite_float(record.get("limiting_apparent_magnitude"))
    if np.isfinite(peak_mag) and np.isfinite(limiting_mag):
        visible = bool(peak_mag <= limiting_mag)
    else:
        visible = bool(record.get(
            "is_photometrically_visible",
            record.get("is_physically_detected", False),
        ))
    camera_detected = bool(visible and frames >= minimum_frames)

    record["is_photometrically_visible"] = visible
    record["minimum_detected_frames"] = minimum_frames
    record["meets_minimum_frame_requirement"] = bool(frames >= minimum_frames)
    record["is_camera_detected"] = camera_detected
    # Keep the established CSV key as a compatibility alias, but give it the
    # operational camera-detection meaning from this version onward.
    record["is_physically_detected"] = camera_detected
    record["detection_definition"] = (
        f"at least {minimum_frames} sampled frames with m_app <= camera LM"
    )

    geometry = record.get("geometry")
    if isinstance(geometry, dict):
        geometry["is_photometrically_visible"] = visible
        geometry["minimum_detected_frames"] = minimum_frames
        geometry["meets_minimum_frame_requirement"] = bool(
            frames >= minimum_frames
        )
        geometry["is_camera_detected"] = camera_detected
        geometry["detection_status"] = (
            "camera detected" if camera_detected
            else "visible but insufficient frames" if visible
            else "not visible above camera limit"
        )
        geometry["detection_criteria"] = record["detection_definition"]

    compact_geometry = record.get("geometry_summary")
    if isinstance(compact_geometry, dict):
        compact_geometry["is_photometrically_visible"] = visible
        compact_geometry["minimum_detected_frames"] = minimum_frames
        compact_geometry["meets_minimum_frame_requirement"] = bool(
            frames >= minimum_frames
        )
        compact_geometry["is_camera_detected"] = camera_detected
...
def make_event_data_record(
    ...
    minimum_frames = _normalise_minimum_detected_frames(
        geometry.get("minimum_detected_frames", 10)
    )
    is_photometrically_visible = bool(geometry.get(
        "is_photometrically_visible",
        geometry.get("is_physically_detectable", False),
    ))
    is_detected = bool(geometry.get(
        "is_camera_detected",
        is_photometrically_visible and detected_frames >= minimum_frames,
    ))
    ...
    return {
        ...
        "is_photometrically_visible": is_photometrically_visible,
        "minimum_detected_frames": minimum_frames,
        "meets_minimum_frame_requirement": bool(detected_frames >= minimum_frames),
        "is_camera_detected": is_detected,
...
def plot_batch_velocity_mass_summary(
    ...
    out_path: str | Path,
    annotate_frame_counts: bool = True,
    title: str = "Mars meteor detectability summary",
    minimum_detected_frames: int | None = None,
) -> str:
    """
    ...
    if minimum_detected_frames is None:
        frame_requirements = [
            int(record.get("minimum_detected_frames", 1) or 1)
            for record in records if isinstance(record, dict)
        ]
        minimum_detected_frames = (
            int(np.median(frame_requirements)) if frame_requirements else 1
        )
    minimum_detected_frames = _normalise_minimum_detected_frames(
        minimum_detected_frames, default=1
    )
    valid_records = [
    ...
    ax.set_title(
        f"{title}\nCamera detection requires >= "
        f"{minimum_detected_frames} frames with m_app <= LM"
    )
    ...
    legend_handles = [
        Line2D(
            ...
            label=(
                f"Camera detected (>= {minimum_detected_frames} frames above LM)"
            ),
        ),
        *([Line2D(
            [0], [0], linestyle="none", color="none",
            label="Number beside point = detected frames",
        )] if annotate_frame_counts else []),
    ]
...
def _normal_cdf(values: np.ndarray | float) -> np.ndarray:
    """Standard-normal cumulative distribution without requiring SciPy."""
    values = np.asarray(values, dtype=float)
    erf_values = np.fromiter(
        (math.erf(float(value)/math.sqrt(2.0)) for value in values.ravel()),
        dtype=float,
        count=values.size,
...
def _detection_likelihood_arrays(
    event_records: Iterable[dict[str, Any]],
) -> dict[str, Any]:
    """Extract the numerical arrays used by the empirical likelihood model."""
    records = list(event_records)
    mass = np.asarray([
        _finite_float(record.get("initial_mass_kg")) for record in records
    ], dtype=float)
    speed = np.asarray([
        _finite_float(record.get("mars_velocity_kms")) for record in records
    ], dtype=float)
    peak_mag = np.asarray([
        _finite_float(record.get("peak_apparent_magnitude")) for record in records
    ], dtype=float)
    limiting_mag = np.asarray([
        _finite_float(record.get("limiting_apparent_magnitude")) for record in records
    ], dtype=float)
    detected = np.asarray([
        bool(record.get(
            "is_camera_detected", record.get("is_physically_detected", False)
    ...
    frames = np.asarray([
        int(record.get("detected_frame_count", 0) or 0) for record in records
    ], dtype=int)
...
def _sigmoid(values: np.ndarray | float) -> np.ndarray:
    """Numerically stable logistic function for scalar or array inputs."""
    values = np.asarray(values, dtype=float)
...
def _fit_regularized_camera_logistic(
    ...
    """Fit P(camera detection) from log-mass and log-speed using NumPy only."""
...
def fit_detection_likelihood_model(
    event_records: Iterable[dict[str, Any]],
    huber_tuning: float = 1.345,
    max_iterations: int = 100,
    tolerance: float = 1.0e-10,
    minimum_sigma_mag: float = 0.15,
    minimum_detected_frames: int | None = None,
) -> dict[str, Any]:
    """
    Fit a robust empirical relation between mass, Mars speed and peak m_app.

    The fitted equation is::

        peak_m_app = a + b_m log10(mass_kg) + b_v log10(speed_km_s)

    The peak-magnitude relation is retained for interpretation. A regularized
    logistic component is also fitted directly to the operational camera label:
    at least ``minimum_detected_frames`` sampled frames above the camera LM.
    This model is an empirical summary of the simulation ensemble, not a
    substitute for MetSim.
    """
    arrays = _detection_likelihood_arrays(event_records)
    valid = arrays["valid"]
    if np.count_nonzero(valid) < 6:
        raise ValueError(
            "At least six meteors with finite mass, Mars speed and peak apparent "
            "magnitude are required to fit the detection-likelihood model."
        )

    mass = arrays["mass"][valid]
    speed = arrays["speed"][valid]
    peak_mag = arrays["peak_mag"][valid]
    detected = arrays["detected"][valid]
    limiting_mag = arrays["limiting_mag"][valid]
    ...
    coefficients = np.linalg.lstsq(design, peak_mag, rcond=None)[0]

    iterations = 0
    for iterations in range(1, int(max_iterations) + 1):
        residual = peak_mag - design @ coefficients
        residual_median = float(np.median(residual))
        sigma = 1.4826*float(np.median(np.abs(residual - residual_median)))
        sigma = max(sigma, float(minimum_sigma_mag), np.finfo(float).eps)

        scaled = (residual - residual_median)/(float(huber_tuning)*sigma)
        weights = np.ones_like(scaled)
        outliers = np.abs(scaled) > 1.0
        weights[outliers] = 1.0/np.abs(scaled[outliers])

        weighted_design = design*np.sqrt(weights)[:, None]
        weighted_target = peak_mag*np.sqrt(weights)
        updated = np.linalg.lstsq(
            weighted_design, weighted_target, rcond=None
        )[0]

        if np.max(np.abs(updated - coefficients)) < float(tolerance):
            coefficients = updated
            break
        coefficients = updated

    predicted = design @ coefficients
    residual = peak_mag - predicted
    residual_median = float(np.median(residual))
    residual_sigma = 1.4826*float(
        np.median(np.abs(residual - residual_median))
    )
    residual_sigma = max(residual_sigma, float(minimum_sigma_mag))
    rmse_mag = float(np.sqrt(np.mean(residual**2)))

    finite_lm = limiting_mag[np.isfinite(limiting_mag)]
    representative_lm = (
        float(np.median(finite_lm)) if finite_lm.size else 4.0
    )

    if minimum_detected_frames is None:
        frame_requirements = [
            int(record.get("minimum_detected_frames", 1) or 1)
            for record in arrays["records"] if isinstance(record, dict)
        ]
        minimum_detected_frames = (
            int(np.median(frame_requirements)) if frame_requirements else 1
        )
    ...
    logistic_model = _fit_regularized_camera_logistic(
        mass=mass, speed=speed, detected=detected
    )

    b_mass = float(coefficients[1])
    b_speed = float(coefficients[2])
    equivalent_speed_exponent = (
        float(b_speed/b_mass)
    ...
    return {
        "format_version": 2,
        "model_type": (
            "camera_detection_logistic_plus_peak_magnitude"
            if logistic_model is not None
            else "robust_peak_magnitude_mass_speed_fallback"
        ),
        "equation": (
            "peak_m_app = intercept + b_mass*log10(mass_kg) "
            "+ b_speed*log10(speed_km_s)"
        ),
        "intercept": float(coefficients[0]),
        "b_log10_mass": b_mass,
        "b_log10_speed": b_speed,
        "residual_median_mag": residual_median,
        "residual_sigma_mag": residual_sigma,
        "rmse_mag": rmse_mag,
        "representative_limiting_magnitude": representative_lm,
        "sample_count": int(len(mass)),
        "detected_sample_count": int(np.count_nonzero(detected)),
        "minimum_detected_frames": int(minimum_detected_frames),
        "camera_detection_definition": (
            f"at least {minimum_detected_frames} sampled frames with m_app <= camera LM"
        ),
        "camera_logistic_model": logistic_model,
        "mass_range_kg": [float(np.min(mass)), float(np.max(mass))],
        "speed_range_kms": [float(np.min(speed)), float(np.max(speed))],
        "equivalent_luminous_proxy": "mass_kg * speed_km_s**gamma",
        "equivalent_speed_exponent_gamma": equivalent_speed_exponent,
        "huber_tuning": float(huber_tuning),
        "fit_iterations": int(iterations),
    }


def predict_peak_apparent_magnitude(
    initial_mass_kg: np.ndarray | float,
    mars_velocity_kms: np.ndarray | float,
    model: dict[str, Any],
) -> np.ndarray:
    """Predict peak apparent magnitude for one or many mass-speed pairs."""
    mass = np.asarray(initial_mass_kg, dtype=float)
    speed = np.asarray(mars_velocity_kms, dtype=float)
    ...
    with np.errstate(invalid="ignore", divide="ignore"):
        prediction_values = (
            ...
            + float(model["b_log10_mass"])*np.log10(mass)
            + float(model["b_log10_speed"])*np.log10(speed)
        )
    return np.asarray(np.where(valid, prediction_values, np.nan), dtype=float)
...
def predict_detection_probability(
    ...
    mars_velocity_kms: np.ndarray | float,
    model: dict[str, Any],
    limiting_apparent_magnitude: np.ndarray | float | None = None,
) -> np.ndarray:
    """Estimate operational camera-detection probability from mass and speed."""
    mass = np.asarray(initial_mass_kg, dtype=float)
    speed = np.asarray(mars_velocity_kms, dtype=float)
    mass, speed = np.broadcast_arrays(mass, speed)
    valid = (
    ...
    representative_lm = float(model["representative_limiting_magnitude"])
    requested_lm = (
        representative_lm if limiting_apparent_magnitude is None
        else np.asarray(limiting_apparent_magnitude, dtype=float)
    )
    requested_lm = np.broadcast_to(requested_lm, mass.shape)

    logistic_model = model.get("camera_logistic_model")
    use_camera_logistic = (
        isinstance(logistic_model, dict)
        and np.all(np.isfinite(requested_lm))
        and np.allclose(requested_lm, representative_lm, atol=1.0e-9, rtol=0.0)
    )
    if use_camera_logistic:
        coefficients = np.asarray(logistic_model["coefficients"], dtype=float)
        center = np.asarray(logistic_model["feature_center"], dtype=float)
        scale = np.asarray(logistic_model["feature_scale"], dtype=float)
        with np.errstate(invalid="ignore", divide="ignore"):
            log_mass = np.log10(mass)
            log_speed = np.log10(speed)
            standardized_mass = (log_mass - center[0])/scale[0]
            standardized_speed = (log_speed - center[1])/scale[1]
            linear = (
                coefficients[0]
        ...
        # Fallback supports testing a different LM, although it represents peak
        # visibility rather than the trained minimum-frame camera requirement.
        predicted_mag = predict_peak_apparent_magnitude(mass, speed, model)
        sigma = max(float(model["residual_sigma_mag"]), np.finfo(float).eps)
        probability = _normal_cdf((requested_lm - predicted_mag)/sigma)
        valid = valid & np.isfinite(predicted_mag) & np.isfinite(requested_lm)
...
def plot_detection_likelihood_surface(
    event_records: Iterable[dict[str, Any]],
    model: dict[str, Any],
    out_path: str | Path,
    limiting_apparent_magnitude: float | None = None,
    grid_size: int = 240,
    annotate_frame_counts: bool = False,
) -> str:
    """Plot detection probability over the Mars velocity-initial mass plane."""
    arrays = _detection_likelihood_arrays(event_records)
    valid = arrays["valid"]
    if not np.any(valid):
        raise ValueError(
            "No valid meteors are available for the likelihood surface."
        )

    mass = arrays["mass"][valid]
    speed = arrays["speed"][valid]
    detected = arrays["detected"][valid]
    frames = arrays["frames"][valid]
    minimum_frames = _normalise_minimum_detected_frames(
        model.get("minimum_detected_frames", 1), default=1
    )

    if limiting_apparent_magnitude is None:
        limiting_apparent_magnitude = float(
            model["representative_limiting_magnitude"]
        )
    limiting_apparent_magnitude = float(limiting_apparent_magnitude)

    speed_min = max(0.1, float(np.min(speed))*0.92)
    speed_max = float(np.max(speed))*1.08
    log_mass_min = float(np.log10(np.min(mass))) - 0.25
    log_mass_max = float(np.log10(np.max(mass))) + 0.25

    speed_grid = np.linspace(speed_min, speed_max, int(grid_size))
    mass_grid = np.logspace(log_mass_min, log_mass_max, int(grid_size))
    velocity_mesh, mass_mesh = np.meshgrid(speed_grid, mass_grid)
    probability = predict_detection_probability(
        mass_mesh,
        velocity_mesh,
        model,
        limiting_apparent_magnitude=limiting_apparent_magnitude,
    )
    ...
    contours = ax.contour(
        ...
        linewidths=[1.1, 2.0, 1.1],
    )
    ax.clabel(
        contours,
        fmt={0.1: "10%", 0.5: "50%", 0.9: "90%"},
        fontsize=9,
    )

    ax.scatter(
        speed[~detected], mass[~detected],
        s=30, facecolors="white", edgecolors="black",
        linewidths=0.7, alpha=0.85, zorder=4,
        label="Simulated but not detected",
    )
    if np.any(detected):
        ax.scatter(
            speed[detected], mass[detected],
            s=52, facecolors="white", edgecolors="black",
            linewidths=0.7, alpha=0.95, zorder=5,
        )
        ax.scatter(
            speed[detected], mass[detected],
            s=105, facecolors="none", edgecolors="red",
            linewidths=1.8, zorder=6,
            label=f"Camera detected (>= {minimum_frames} frames)",
        )

    if annotate_frame_counts:
        for velocity, meteor_mass, frame_count in zip(speed, mass, frames):
            ax.annotate(
                str(int(frame_count)),
                (velocity, meteor_mass),
    ...
    colorbar.set_label(
        f"Estimated probability of camera detection (>= {minimum_frames} frames)"
    )
    probability_ticks = np.linspace(0.0, 1.0, 6)
    colorbar.set_ticks(probability_ticks)
    colorbar.set_ticklabels([
        f"{int(value*100)}%" for value in probability_ticks
    ])

    gamma = _finite_float(model.get("equivalent_speed_exponent_gamma"))
    ax.set_yscale("log")
    ax.set_xlabel("Initial velocity at Mars [km/s]")
    ax.set_ylabel("Initial meteoroid mass [kg]")
    ax.set_title(
        "Empirical METEORCAM camera-detection likelihood\n"
        f"LM = {limiting_apparent_magnitude:.1f}; minimum = "
        f"{minimum_frames} frames; luminous proxy approximately m v^{gamma:.2f}"
    )
    ...
    fig.text(
        0.02, 0.015,
        (
            f"Robust peak-magnitude fit: sigma="
            f"{model['residual_sigma_mag']:.2f} mag; "
            f"N={model['sample_count']}; "
            f"camera detected={model['detected_sample_count']} using >= "
            f"{minimum_frames} frames. "
            "The surface is fitted directly to the camera-detection labels when "
            "both detected and non-detected samples are available."
        ),
        fontsize=8.5,
    )
...
def save_detection_likelihood_csv(
    event_records: Iterable[dict[str, Any]],
    model: dict[str, Any],
    out_path: str | Path,
    limiting_apparent_magnitude: float | None = None,
) -> str:
    """Save the mass-speed table with energy and predicted likelihood columns."""
    records = list(event_records)
    columns = [
        "event_name",
        "source_event_name",
        "sample_kind",
        "synthetic_speed_index",
        "synthetic_speed_kms",
        "source_native_mars_velocity_kms",
        "json_kind",
        "initial_mass_kg",
        "earth_velocity_kms",
        "mars_velocity_kms",
        ...
        "predicted_peak_apparent_magnitude",
    ...
    with out_path.open("w", newline="", encoding="utf-8") as fh:
        ...
        writer.writeheader()
        for record in records:
            mass = _finite_float(record.get("initial_mass_kg"))
            speed = _finite_float(record.get("mars_velocity_kms"))
            event_lm = _finite_float(
                record.get("limiting_apparent_magnitude")
            )
            if limiting_apparent_magnitude is not None:
                event_lm = float(limiting_apparent_magnitude)
            elif not np.isfinite(event_lm):
                event_lm = float(
                    model["representative_limiting_magnitude"]
                )

            kinetic_energy = (
                0.5*mass*(speed*1000.0)**2
                if np.isfinite(mass) and mass > 0.0
                and np.isfinite(speed) and speed > 0.0
                else np.nan
            )
            predicted_mag = float(
                predict_peak_apparent_magnitude(mass, speed, model)
            )
            probability = float(
                predict_detection_probability(
                    ...
                    speed,
                    model,
                    limiting_apparent_magnitude=event_lm,
                )
            )
            ...
            row.update({
                ...
                "limiting_apparent_magnitude": event_lm,
                "predicted_peak_apparent_magnitude": predicted_mag,
                "predicted_detection_probability": probability,
                "event_data_pickle": outputs.get("event_data_pickle", ""),
            })
            writer.writerow({
                key: _csv_scalar(value) for key, value in row.items()
            })
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--input",
        default=r"C:\Users\maxiv\Documents\UWO\Papers\0.5)METEORCAM-Strawman\All",
        help=(
    ...
    parser.add_argument(
        "--batch-likelihood-plot-name",
        default="Mars_detection_likelihood_surface.png",
        help="Filename for the empirical mass-speed detection-likelihood plot.",
    )
    parser.add_argument(
        "--batch-likelihood-model-name",
        default="Mars_detection_likelihood_model.pkl",
        help="Filename for the fitted empirical likelihood-model pickle.",
    )
    parser.add_argument(
        "--batch-likelihood-csv-name",
        default="Mars_detection_velocity_mass_with_likelihood.csv",
        help="Filename for the CSV augmented with kinetic energy and likelihood.",
    )
    parser.add_argument(
        "--likelihood-limiting-mag",
        type=float, default=None,
        help=(
            "Optional limiting magnitude used for the likelihood surface. "
            "Defaults to the median camera LM in the processed sample."
        ),
    )
    parser.add_argument(
        "--likelihood-annotate-frames",
        action="store_true",
        help="Annotate likelihood-plot points with detected-frame counts.",
    )
    parser.add_argument(
        "--synthetic-speed-sweep",
        action=argparse.BooleanOptionalAction,
        default=True,
        help=(
            "Rerun each fitted meteoroid over a Mars-speed grid while keeping "
            "its fitted mass, density, ablation, erosion, fragmentation, and "
            "trajectory parameters unchanged. Dynamic-pressure trigger heights "
            "are remapped separately at every synthetic speed."
        ),
    )
    parser.add_argument(
        "--synthetic-speed-min-kms",
        type=float,
        default=MARS_METEOR_MIN_SPEED_KMS,
        help="Minimum Mars speed in the synthetic sweep. Default: 4.926 km/s.",
    )
    parser.add_argument(
        "--synthetic-speed-max-kms",
        type=float,
        default=MARS_METEOR_MAX_SPEED_KMS,
        help="Maximum Mars speed in the synthetic sweep. Default: 58.254 km/s.",
    )
    parser.add_argument(
        "--synthetic-speed-count",
        type=int,
        default=12,
        help=(
            "Number of linearly spaced Mars speeds per fitted meteoroid when "
            "--synthetic-speed-values-kms is not supplied. Default: 12."
        ),
    )
    parser.add_argument(
        "--synthetic-speed-values-kms",
        default='5,10,15,20,30,40,50,58',
        help=(
            "Optional comma-separated Mars speeds, e.g. '5,10,15,20,30,40,50,58'. "
            "When supplied, this overrides min/max/count."
        ),
    )
    parser.add_argument(
        "--synthetic-speed-seed",
        type=int,
        default=0,
        help=(
            "Camera-phase seed shared by all speeds in a sweep. Keeping it fixed "
            "reduces random frame-phase noise when comparing velocities."
        ),
    )
    parser.add_argument(
        "--synthetic-workers",
        ...
        default=0,
        help=(
            "Number of worker processes used for synthetic sweeps. Use 0 to use "
            "all CPUs allocated to the current machine or scheduler job, 1 for "
            "serial execution, or any positive integer for an explicit limit."
        ),
    )
    parser.add_argument(
        ...
        default="spawn",
        help=(
            "Multiprocessing start method. 'spawn' is the safest cross-platform "
            "default; 'fork' can start faster on Linux but may be less robust "
            "with scientific libraries."
        ),
    )
    parser.add_argument(
        "--synthetic-worker-threads",
        type=int,
        default=1,
        help=(
            "Thread limit requested inside each worker for OpenMP/BLAS libraries. "
            "The default of 1 avoids oversubscribing the node when many worker "
            "processes run simultaneously."
        ),
    )
    parser.add_argument(
        "--synthetic-speed-timeout-seconds",
        type=float,
        default=90.0,
        help=(
            "Hard wall-clock timeout for one synthetic Mars-speed case. A value "
            "of 900 gives each speed 15 minutes. Use 0 to disable the timeout."
        ),
    )
    parser.add_argument(
        "--synthetic-timeout-grace-seconds",
        type=float,
        default=5.0,
        help=(
            "Seconds allowed after terminate() before a stalled synthetic-speed "
            "process is force-killed. Default: 5."
        ),
    )
    parser.add_argument(
        "--synthetic-timeout-start-method",
        choices=("auto", "spawn", "fork", "forkserver"),
        default="auto",
        help=(
            "Start method for the disposable per-speed timeout process. 'auto' "
            "uses fork on POSIX when available and spawn otherwise."
        ),
    )
    parser.add_argument(
        "--retry-synthetic-timeouts",
        action="store_true",
        help=(
            "Retry speed cases previously marked as timed out in an existing "
            "per-event cache. By default, cached timeouts are skipped."
        ),
    )
    parser.add_argument(
        "--synthetic-speed-summary-name",
        default="Mars_detection_synthetic_speed_sweep.pkl",
        help="Global pickle containing all synthetic speed-sweep cases.",
    )
    parser.add_argument(
        "--synthetic-speed-csv-name",
        default="Mars_detection_synthetic_speed_sweep.csv",
        help="Flat CSV containing one row per synthetic speed case.",
    )
    parser.add_argument(
        "--synthetic-speed-plot-name",
        default="Mars_detection_synthetic_velocity_mass.png",
        help="Velocity-mass plot containing the synthetic speed cases.",
    )
    parser.add_argument(
        "--synthetic-likelihood-plot-name",
        default="Mars_detection_synthetic_likelihood_surface.png",
        help="Detection-likelihood surface fitted to original plus synthetic cases.",
    )
    parser.add_argument(
        "--synthetic-likelihood-model-name",
        default="Mars_detection_synthetic_likelihood_model.pkl",
        help="Pickle containing the expanded likelihood model.",
    )
    parser.add_argument(
        "--synthetic-likelihood-csv-name",
        default="Mars_detection_synthetic_with_likelihood.csv",
        help="Expanded original-plus-synthetic CSV with predicted likelihood values.",
    )
    parser.add_argument(
        "--rebuild-batch",
        action="store_true",
    ...
    parser.add_argument(
        ...
        default=10,
        help=(
            "Minimum number of sampled camera frames satisfying m_app <= LM "
            "required to classify a meteor as detected. Default: 10. Use 11 "
            "for a strict 'more than 10 frames' criterion."
        ),
    )
...
def available_process_cpu_count() -> int:
    """
    Return the number of CPUs available to this process.

    CPU affinity is preferred because it respects Linux containers and many
    scheduler allocations. Common scheduler environment variables are used as
    additional upper bounds when present.
    """
    candidates: list[int] = []
    ...
    if machine_count is not None and int(machine_count) > 0:
        candidates.append(int(machine_count))
    ...
    for variable in (
        ...
        if not value:
            continue
        # SLURM_CPUS_ON_NODE can occasionally contain values such as "48(x2)".
...
def resolve_synthetic_worker_count(
    requested_workers: int,
    event_count: int,
) -> int:
    """Resolve the requested process count and never create idle workers."""
    requested = int(requested_workers)
    if requested < 0:
        raise ValueError("--synthetic-workers must be 0 or a positive integer.")

    available = available_process_cpu_count()
    workers = available if requested == 0 else requested
    return max(1, min(int(workers), max(1, int(event_count))))


def _limit_worker_library_threads(thread_count: int) -> None:
    """Reduce OpenMP/BLAS oversubscription inside a multiprocessing worker."""
    ...
    # If threadpoolctl is installed, also update already-loaded native pools.
    try:
...
def _serialisable_pair_for_worker(pair: dict[str, Any]) -> dict[str, Any]:
    """Convert a discovered input-pair mapping into spawn-safe primitives."""
    return {
        "event_name": str(pair["event_name"]),
        "pickle_path": str(pair["pickle_path"]),
        "json_path": str(pair["json_path"]),
        "event_directory": str(pair["event_directory"]),
        "json_kind": pair.get("json_kind"),
        "json_layout": pair.get("json_layout"),
    }


def _synthetic_event_worker(
    args_values: dict[str, Any],
    pair: dict[str, Any],
    speed_values_kms: list[float],
) -> dict[str, Any]:
    """
    Worker entry point for one fitted meteoroid.

    Each process performs the complete speed sweep for one source event. This
    keeps the shared Earth run local to that event and prevents nested process
    pools. The worker writes only that event's own cache, so workers do not
    contend for the same output file.
    """
    worker_threads = int(args_values.get("synthetic_worker_threads", 1))
    _limit_worker_library_threads(worker_threads)

    worker_args = argparse.Namespace(**args_values)
    worker_args.input = None
    worker_args.pickle = str(pair["pickle_path"])
    worker_args.json = str(pair["json_path"])
    worker_args.output_dir = str(pair["event_directory"])
    worker_args.base_name = str(pair["event_name"])
    worker_args.json_kind = pair.get("json_kind")
    worker_args.json_layout = pair.get("json_layout")

    try:
        sweep = run_synthetic_speed_sweep_for_event(
            worker_args,
            np.asarray(speed_values_kms, dtype=float),
        )
        return {
            "ok": True,
            "source_event_name": str(pair["event_name"]),
            "records": list(sweep.get("records", [])),
            "failures": list(sweep.get("failures", [])),
            "cache_path": str(
                Path(pair["event_directory"])
                / f"{pair['event_name']}_Mars_synthetic_speed_sweep.pkl"
            ),
        }
    except Exception as exc:
        return {
...
def synthetic_speed_grid_from_args(args: argparse.Namespace) -> np.ndarray:
    """Return the requested unique, sorted Mars-speed grid in km/s."""
    explicit = getattr(args, "synthetic_speed_values_kms", None)
    if explicit is not None and str(explicit).strip():
        tokens = str(explicit).replace(";", ",").replace(" ", ",").split(",")
        values = [float(token) for token in tokens if token.strip()]
        speeds = np.asarray(values, dtype=float)
    else:
        count = int(getattr(args, "synthetic_speed_count", 12))
        if count < 2:
            raise ValueError("--synthetic-speed-count must be at least 2.")
        minimum = float(getattr(args, "synthetic_speed_min_kms", MARS_METEOR_MIN_SPEED_KMS))
        maximum = float(getattr(args, "synthetic_speed_max_kms", MARS_METEOR_MAX_SPEED_KMS))
        if not np.isfinite(minimum) or not np.isfinite(maximum) or maximum <= minimum:
            raise ValueError(
                f"Invalid synthetic Mars-speed interval: min={minimum}, max={maximum}."
            )
        speeds = np.linspace(minimum, maximum, count, dtype=float)

    speeds = speeds[np.isfinite(speeds) & (speeds > 0.0)]
    speeds = np.unique(np.round(speeds, decimals=9))
    if speeds.size == 0:
        raise ValueError("No positive finite synthetic Mars speeds were supplied.")
    return np.sort(speeds)


def _file_mtime_ns(path: str | Path) -> int | None:
    """Return a source-file modification timestamp for cache invalidation."""
    try:
...
def _synthetic_speed_sweep_signature(
    args: argparse.Namespace,
    speed_values_kms: np.ndarray,
) -> dict[str, Any]:
    """Build the configuration signature stored with each speed-sweep cache."""
    return {
        "format_version": 1,
        "pickle_path": _normalised_path_key(args.pickle),
        "json_path": _normalised_path_key(args.json),
        "pickle_mtime_ns": _file_mtime_ns(args.pickle),
        "json_mtime_ns": _file_mtime_ns(args.json),
        "speed_values_kms": [float(value) for value in speed_values_kms],
        ...
        "minimum_detected_frames": int(args.minimum_detected_frames),
        ...
        "synthetic_speed_seed": int(args.synthetic_speed_seed),
        # Timeout settings are operational rather than scientific and therefore
        # intentionally do not invalidate successfully completed cached cases.
        "postpeak_only": bool(args.postpeak_only),
        "use_all_cameras": bool(args.use_all_cameras),
        "prior_path": _normalised_path_key(args.prior) if args.prior else "",
        "P_0m_prior": _finite_float(args.P_0m_prior),
        "atm_coeff_json": _normalised_path_key(args.atm_coeff_json) if args.atm_coeff_json else "",
        "atm_coeff_npy": _normalised_path_key(args.atm_coeff_npy) if args.atm_coeff_npy else "",
        "atm_min_km": float(args.atm_min_km),
        "atm_max_km": float(args.atm_max_km),
        "atm_step_m": float(args.atm_step_m),
        "h_kill_km": float(args.h_kill_km),
    }


def _synthetic_sweep_cache_matches(
    cached: Any,
    signature: dict[str, Any],
) -> bool:
    """Check whether a saved per-event speed sweep can be reused."""
    if not isinstance(cached, dict):
        return False
    return cached.get("signature") == signature


def _compact_synthetic_geometry(geometry: dict[str, Any]) -> dict[str, Any]:
    """Retain useful scalar detection diagnostics without storing large arrays."""
    keys = (
        "peak_abs_mag",
        "peak_app_mag",
        "peak_altitude_km",
        ...
        "central_angle_deg",
        "longitude_regime",
        "longitude_speed_fraction",
        "meteor_speed_kms",
        "design_delta_duration_s",
        "design_delta_frame_count",
        "design_delta_fully_within_camera_limit",
    )
    compact = {key: geometry.get(key) for key in keys}
    compact["sampled_detected_mask"] = np.asarray(
        geometry.get("sampled_detected_mask", []), dtype=bool
    ).tolist()
    compact["sampled_app_mag"] = np.asarray(
        geometry.get("sampled_app_mag", []), dtype=float
    ).tolist()
    compact["sampled_time_s"] = np.asarray(
        geometry.get("sampled_time_s", []), dtype=float
    ).tolist()
    return compact



def _resolve_timeout_start_method(requested: str) -> str:
    """Resolve the disposable per-speed process start method."""
    requested = str(requested).strip().lower()
    available = set(mp.get_all_start_methods())

    if requested == "auto":
        if os.name != "nt" and "fork" in available:
            return "fork"
        return "spawn"
    ...
    if requested not in available:
        raise ValueError(
            ...
            f"available methods are {sorted(available)}."
        )
    return requested


def _run_one_synthetic_speed_case(
    case_args_values: dict[str, Any],
    speed_index: int,
    speed_kms: float,
    base_name: str,
    output_dir: str | Path,
    ...
    trajectories: list[Any],
    dens_co_new: np.ndarray,
    native_speed_kms: float,
) -> dict[str, Any]:
    """Run one speed case and return its compact synthetic-event record."""
    case_args = argparse.Namespace(**case_args_values)
    case_args.v_init_kms = float(speed_kms)
    case_args.sampling_seed = int(case_args.synthetic_speed_seed)

    mars_reference_const = build_density_mapped_planet_const(
        earth_const=earth_result.const,
        obs_data=obs_data,
        trajectories=trajectories,
        dens_co_new=dens_co_new,
        args=case_args,
    )
    mars_reference_result = run_model_raw(mars_reference_const)

    (
        mars_dynamic_const,
        _erosion_start_mars,
        _erosion_change_mars,
        _erosion_change_q,
    ) = build_dynamic_pressure_trigger_const(
        best_guess_obj_plot=earth_result,
        best_guess_obj_plot_mars=mars_reference_result,
        best_guess_cost_mars=mars_reference_const,
    )
    mars_dynamic_result = run_model_raw(mars_dynamic_const)
    maybe_integrate_luminosity(mars_dynamic_result, obs_data)

    segment = extract_simulation_segment(mars_dynamic_result)
    full_geometry = build_detected_track_geometry(
        segment,
        zenith_angle_rad=float(mars_dynamic_result.const.zenith_angle),
        planet_radius_km=float(case_args.planet_radius_km),
        camera_altitude_km=float(case_args.camera_altitude_km),
        central_angle_deg=float(case_args.central_angle_deg),
        traj_azimuth_deg=float(case_args.traj_azimuth_deg),
        selected_altitude_km=case_args.selected_altitude_km,
        physically_motivated_longitude=bool(
            case_args.physically_motivated_longitude
        ),
        meteor_speed_kms=float(speed_kms),
        mars_orbital_speed_kms=float(case_args.mars_orbital_speed_kms),
    )
    detected_geometry = apply_apparent_detection_and_camera_sampling(
        full_geometry,
        det_mag_cut=case_args.det_mag_cut,
        limiting_app_mag=case_args.limiting_app_mag,
        use_delta_mag_cut=not case_args.no_delta_mag_cut,
        from_peak_only=bool(case_args.postpeak_only),
        camera_fps=float(case_args.camera_fps),
        max_dots=case_args.n_detected_dots,
        sampling_seed=int(case_args.synthetic_speed_seed),
        minimum_detected_frames=int(case_args.minimum_detected_frames),
    )

    synthetic_name = f"{base_name}__Mars_v{float(speed_kms):06.3f}_kms"
    record = make_event_data_record(
        event_name=synthetic_name,
        pickle_path=case_args.pickle,
        json_path=case_args.json,
        output_dir=output_dir,
        earth_const=earth_result.const,
        mars_const=mars_dynamic_result.const,
        geometry=detected_geometry,
        output_paths={"speed_sweep_pickle": str(cache_path)},
        json_kind=getattr(case_args, "json_kind", None),
        json_layout=getattr(case_args, "json_layout", None),
    )
    record["record_format_version"] = 4
    record["source_event_name"] = str(base_name)
    record["sample_kind"] = "synthetic_speed_sweep"
    record["synthetic_speed_index"] = int(speed_index)
    record["synthetic_speed_kms"] = float(speed_kms)
    record["source_native_mars_velocity_kms"] = float(native_speed_kms)
    record["physical_parameters_held_fixed"] = [
        "initial_mass",
        "bulk_density",
        "ablation_coefficient",
        "shape_density_coefficient",
        "luminous_efficiency_model",
        "erosion_coefficients",
        "grain_mass_distribution",
        "fragmentation_mass_fractions",
        "entry_zenith_angle",
    ]
    record["geometry_summary"] = _compact_synthetic_geometry(detected_geometry)
    record["lightcurve_overlay"] = _compact_lightcurve_overlay_data(mars_dynamic_result)
    record.pop("geometry", None)
    return record


def _synthetic_speed_case_process_entry(
    result_path: str,
    case_args_values: dict[str, Any],
    speed_index: int,
    speed_kms: float,
    ...
    """Disposable child process used to enforce a hard per-speed timeout."""
    try:
        record = _run_one_synthetic_speed_case(
            case_args_values=case_args_values,
            ...
            trajectories=trajectories,
            dens_co_new=dens_co_new,
            native_speed_kms=native_speed_kms,
        )
    ...
    try:
        save_pickle_data(payload, result_path)
    except BaseException:
        # The parent also checks the process exit code and missing result file.
        pass


def _run_synthetic_speed_case_with_timeout(
    *,
    case_args_values: dict[str, Any],
    speed_index: int,
    speed_kms: float,
    ...
    """Run one speed in a disposable process and terminate it on timeout."""
    ...
    method = _resolve_timeout_start_method(start_method)
    context = mp.get_context(method)
    fd, temporary_name = tempfile.mkstemp(
        prefix=f".{base_name}_v{float(speed_kms):06.3f}_",
        suffix=".timeout_result.pkl",
        dir=str(output_dir),
    )
    os.close(fd)
    ...
    process = context.Process(
        target=_synthetic_speed_case_process_entry,
        args=(
            str(temporary_path),
            case_args_values,
            int(speed_index),
            ...
            obs_data,
            trajectories,
            np.asarray(dens_co_new, dtype=float),
            float(native_speed_kms),
        ),
        daemon=False,
    )
    ...
    try:
        ...
        if process.is_alive():
            process.terminate()
            process.join(max(0.0, float(grace_seconds)))
            ...
            return {
                "status": "timeout",
                "error": (
                    f"Synthetic speed case exceeded {timeout_seconds:.1f} s "
                    "and was terminated."
                ),
        ...
        if temporary_path.exists():
            payload = load_pickle_data(temporary_path)
            if isinstance(payload, dict) and payload.get("ok", False):
                return {"status": "ok", "record": payload["record"]}
            if isinstance(payload, dict):
                return {
                    "status": "failed",
                    "error": payload.get("error", "Unknown child-process error"),
        ...
        return {
            "status": "failed",
            "error": (
                "Synthetic speed child exited without producing a result "
        ...
        return {
            "status": "failed",
            "error": f"Could not run timeout child: {type(exc).__name__}: {exc}",
            "traceback": traceback.format_exc(),
        }
    finally:
        try:
...
def run_synthetic_speed_sweep_for_event(
    args: argparse.Namespace,
    speed_values_kms: np.ndarray,
) -> dict[str, Any]:
    """
    Rerun one fitted meteoroid over a Mars-speed grid.

    The Earth fit and all fitted physical parameters are held fixed. For every
    Mars speed, the same-density reference simulation is rerun and the global
    erosion/fragmentation release heights are remapped to the Mars altitudes
    having the corresponding Earth dynamic pressures. Compact numerical records
    are cached, and the usual _Lightcurve_Earth_vs_Mars.png plot is updated to
    overlay the synthetic Mars light curves for quick visual inspection. When
    the per-speed timeout is positive, every velocity case runs in a
    disposable child process so a stalled simulation can be terminated without
    losing the remaining speeds for that fitted meteor.
    """
    if not args.pickle or not args.json:
        raise ValueError("Synthetic speed-sweep mode requires both pickle and JSON inputs.")

    output_dir = Path(
        args.output_dir
        if args.output_dir is not None
        else (os.path.dirname(os.path.abspath(args.pickle)) or ".")
    ).resolve()
    output_dir.mkdir(parents=True, exist_ok=True)
    base_name = args.base_name or Path(args.pickle).stem.replace("_trajectory", "")
    cache_path = output_dir / f"{base_name}_Mars_synthetic_speed_sweep.pkl"
    signature = _synthetic_speed_sweep_signature(args, speed_values_kms)

    cached_payload: dict[str, Any] | None = None
    if cache_path.exists() and not bool(args.rebuild_batch):
        try:
            loaded = load_pickle_data(cache_path)
            if _synthetic_sweep_cache_matches(loaded, signature):
                cached_payload = loaded
                completed = {
                    round(_finite_float(record.get("synthetic_speed_kms")), 9)
                    for record in loaded.get("records", [])
                    if isinstance(record, dict)
                }
                requested = {round(float(value), 9) for value in speed_values_kms}
                cached_timeouts = {
                    round(_finite_float(failure.get("synthetic_speed_kms")), 9)
                    for failure in loaded.get("failures", [])
                    if isinstance(failure, dict)
                    and str(failure.get("status", "")).lower() == "timeout"
                    and bool(failure.get("skip_on_resume", True))
                }
                considered_done = set(completed)
                if not bool(getattr(args, "retry_synthetic_timeouts", False)):
                    considered_done.update(cached_timeouts)
                if requested.issubset(considered_done):
                    print(f"  Loaded complete synthetic speed sweep: {cache_path}")
                    return loaded
                print(
                    f"  Resuming synthetic speed sweep: {len(completed)}/"
                    f"{len(requested)} speeds already cached."
                )
        except Exception as exc:
            print(f"  Could not reuse synthetic speed-sweep cache: {exc}")

    records = list((cached_payload or {}).get("records", []))
    failures = list((cached_payload or {}).get("failures", []))
    completed_speeds = {
        round(_finite_float(record.get("synthetic_speed_kms")), 9)
        for record in records
        if isinstance(record, dict)
    }
    cached_timeout_speeds = {
        round(_finite_float(failure.get("synthetic_speed_kms")), 9)
    ...
    if bool(getattr(args, "retry_synthetic_timeouts", False)):
        # Remove the old timeout markers so the requested cases are genuinely
        # retried and a successful retry does not retain a stale failure entry.
        failures = [
    ...
    print("  Running the shared Earth best-fit simulation for the speed sweep...")
    earth_result = run_model_raw(earth_const)
    dens_co_new = atmosphere_coefficients(args)

    native_speed_mps = estimate_mars_vinit_from_trajectory(trajectories)
    if native_speed_mps is None or not np.isfinite(native_speed_mps):
        native_speed_mps = _first_finite_attribute(
            earth_result.const, ("v_init", "initial_velocity", "v0")
        )
    native_speed_kms = float(native_speed_mps)/1000.0

    for speed_index, speed_kms in enumerate(speed_values_kms):
        rounded_speed = round(float(speed_kms), 9)
        if rounded_speed in completed_speeds:
            continue
        if rounded_speed in cached_timeout_speeds:
            print(
                f"  Skipping previously timed-out speed: "
                f"{float(speed_kms):.3f} km/s "
                "(use --retry-synthetic-timeouts to try it again)."
            )
            continue

        print(
            f"  Synthetic speed {speed_index + 1}/{len(speed_values_kms)}: "
            f"{float(speed_kms):.3f} km/s"
        )
        case_args_values = dict(vars(args))
        outcome = _run_synthetic_speed_case_with_timeout(
            case_args_values=case_args_values,
            ...
            trajectories=trajectories,
            dens_co_new=dens_co_new,
            native_speed_kms=float(native_speed_kms),
            timeout_seconds=float(args.synthetic_speed_timeout_seconds),
            grace_seconds=float(args.synthetic_timeout_grace_seconds),
            start_method=str(args.synthetic_timeout_start_method),
        )

        if outcome.get("status") == "ok":
            records.append(outcome["record"])
            completed_speeds.add(rounded_speed)
            failures = [
                failure for failure in failures
                if not (
                    isinstance(failure, dict)
                    and round(
                        _finite_float(failure.get("synthetic_speed_kms")), 9
                    ) == rounded_speed
                )
            ]
        else:
            ...
            failure = {
                "source_event_name": str(base_name),
                "synthetic_speed_kms": float(speed_kms),
                "status": status,
                "error": outcome.get("error", "Unknown synthetic-speed failure"),
                "timeout_seconds": outcome.get("timeout_seconds"),
                "exitcode": outcome.get("exitcode"),
                "skip_on_resume": status == "timeout",
            }
            # Keep at most one current failure record per speed.
            failures = [
                previous for previous in failures
                if not (
                    isinstance(previous, dict)
                    and round(
                        _finite_float(previous.get("synthetic_speed_kms")), 9
                    ) == rounded_speed
                )
            ]
            ...
            if status == "timeout":
                print(
                    f"    TIMEOUT at {float(speed_kms):.3f} km/s after "
                    f"{float(args.synthetic_speed_timeout_seconds):.1f} s; moving on."
                )
            else:
                print(
                    f"    FAILED at {float(speed_kms):.3f} km/s: "
                    f"{failure['error']}"
                )
        ...
        payload = {
            ...
            "signature": signature,
            "source_event_name": str(base_name),
            "source_native_mars_velocity_kms": float(native_speed_kms),
            "records": records,
            "failures": failures,
            ...
            "synthetic_speed_timeout_seconds": float(
                args.synthetic_speed_timeout_seconds
            ),
        }
        save_pickle_data(payload, cache_path)

    records.sort(key=lambda record: _finite_float(record.get("synthetic_speed_kms")))

    # Update the event light-curve comparison plot with the synthetic Mars curves.
    try:
        native_args = argparse.Namespace(**dict(vars(args)))
        native_args.v_init_kms = None
        mars_reference_const = build_density_mapped_planet_const(
            earth_const=earth_result.const,
            obs_data=obs_data,
            trajectories=trajectories,
        ...
        mars_dynamic_result = run_model_raw(mars_dynamic_const)
        maybe_integrate_luminosity(mars_dynamic_result, obs_data)
        plot_lightcurve_earth_vs_mars_dyn_pressure(
            obs_data=obs_data,
            best_guess_obj_plot=earth_result,
            best_guess_obj_plot_mars=mars_reference_result,
            best_guess_obj_plot_mars_dyn_press=mars_dynamic_result,
            heightsame_dynpress_mars=erosion_start_mars,
            heightsame_dynpress_change_mars=erosion_change_mars,
            output_dir=output_dir,
            base_name=base_name,
            synthetic_curve_records=records,
        )
    except Exception as exc:
        print(f"  Could not update synthetic light-curve overlay plot: {exc}")
    ...
    save_pickle_data(payload, cache_path)
    print(f"  Saved synthetic speed sweep: {cache_path}")
    return payload
...
def run_batch_folder(args: argparse.Namespace, input_folder: str | Path) -> dict[str, Any]:
    ...
    likelihood_plot_path = root / args.batch_likelihood_plot_name
    likelihood_model_path = root / args.batch_likelihood_model_name
    likelihood_csv_path = root / args.batch_likelihood_csv_name
    synthetic_summary_path = root / args.synthetic_speed_summary_name
    synthetic_csv_path = root / args.synthetic_speed_csv_name
    synthetic_plot_path = root / args.synthetic_speed_plot_name
    synthetic_likelihood_plot_path = root / args.synthetic_likelihood_plot_name
    synthetic_likelihood_model_path = root / args.synthetic_likelihood_model_name
    synthetic_likelihood_csv_path = root / args.synthetic_likelihood_csv_name
    ...
    for index, pair in enumerate(valid_pairs, start=1):
        ...
        if record is not None:
            ...
            # and apply the current tunable camera-frame requirement without rerunning.
            record["json_kind"] = pair.get("json_kind")
            record["json_layout"] = pair.get("json_layout")
            apply_minimum_frame_detection_to_record(
                record, args.minimum_detected_frames
            )
    ...
    synthetic_speed_values = np.asarray([], dtype=float)
    if bool(args.synthetic_speed_sweep):
        synthetic_speed_values = synthetic_speed_grid_from_args(args)
        print(
            "\nSynthetic Mars-speed sweep enabled: "
            f"{len(synthetic_speed_values)} speeds per fitted meteoroid, "
            f"{synthetic_speed_values[0]:.3f}--"
            f"{synthetic_speed_values[-1]:.3f} km/s."
        )

        worker_count = resolve_synthetic_worker_count(
            requested_workers=int(args.synthetic_workers),
            event_count=len(valid_pairs),
        )
        available_cpus = available_process_cpu_count()
        print(
            f"Synthetic execution: {worker_count} worker process"
            f"{'es' if worker_count != 1 else ''} for {len(valid_pairs)} events "
            f"({available_cpus} CPUs available; "
            f"{max(1, int(args.synthetic_worker_threads))} native thread(s) per worker)."
        )
        if float(args.synthetic_speed_timeout_seconds) > 0.0:
            print(
                "Per-speed hard timeout: "
                f"{float(args.synthetic_speed_timeout_seconds):.1f} s "
                f"(grace {float(args.synthetic_timeout_grace_seconds):.1f} s; "
                f"start method {args.synthetic_timeout_start_method})."
            )
        else:
            print("Per-speed hard timeout disabled.")

        if worker_count == 1:
            # Keep a straightforward serial path for debugging and small tests.
            for index, pair in enumerate(valid_pairs, start=1):
                print(
                    f"\n[Synthetic {index}/{len(valid_pairs)}] "
                    f"{pair['event_name']}"
                )
                result = _synthetic_event_worker(
                    dict(vars(args)),
                    _serialisable_pair_for_worker(pair),
                    synthetic_speed_values.tolist(),
                )
                synthetic_events.extend(result.get("records", []))
                synthetic_failures.extend(result.get("failures", []))
                if not result.get("ok", False):
                    failure = {
                        "source_event_name": result.get("source_event_name", ""),
                        "pickle_path": result.get("pickle_path", ""),
                        "json_path": result.get("json_path", ""),
                        "error": result.get("error", "Unknown worker failure"),
                    }
                    synthetic_failures.append(failure)
                    print(f"  SYNTHETIC SWEEP FAILED: {failure['error']}")
        else:
            # Parallelize source meteoroids. Each worker runs all requested speeds
            # for one event and reuses that event's shared Earth simulation.
            start_method = str(args.synthetic_start_method)
            if os.name == "nt" and start_method != "spawn":
                print(
                    f"Windows requires the spawn start method; overriding "
                    f"--synthetic-start-method={start_method!r}."
                )
                start_method = "spawn"

            context = mp.get_context(start_method)
            worker_args_values = dict(vars(args))
            worker_pairs = [
                _serialisable_pair_for_worker(pair) for pair in valid_pairs
            ]
            completed_count = 0
            ...
            with ProcessPoolExecutor(
                ...
                for future in as_completed(future_to_pair):
                    ...
                    try:
                        ...
                        result = {
                            "ok": False,
                            "source_event_name": pair["event_name"],
                            "records": [],
                            "failures": [],
                            "pickle_path": pair["pickle_path"],
                    ...
                    if result.get("ok", False):
                        print(
                            f"[Synthetic {completed_count}/{len(worker_pairs)}] "
                            f"completed {result['source_event_name']} "
                            f"({len(result.get('records', []))} cases)."
                        )
                    else:
                        failure = {
                            "source_event_name": result.get(
                                "source_event_name", pair["event_name"]
                            ),
                            "pickle_path": result.get(
                                "pickle_path", pair["pickle_path"]
                            ),
                            "json_path": result.get(
                                "json_path", pair["json_path"]
                            ),
                            "error": result.get(
                        ...
                        print(
                            f"[Synthetic {completed_count}/{len(worker_pairs)}] "
                            f"FAILED {failure['source_event_name']}: "
                            f"{failure['error']}"
                        )

        for synthetic_record in synthetic_events:
            if isinstance(synthetic_record, dict):
                apply_minimum_frame_detection_to_record(
                    synthetic_record, args.minimum_detected_frames
                )

        synthetic_events.sort(
            key=lambda record: (
                str(record.get("source_event_name", "")).casefold(),
                _finite_float(record.get("synthetic_speed_kms")),
            )
        )
        ...
        synthetic_payload = {
            ...
            "speed_values_kms": synthetic_speed_values.tolist(),
            "source_event_count": len(valid_pairs),
            "worker_count": int(worker_count),
            "worker_threads": max(1, int(args.synthetic_worker_threads)),
            "multiprocessing_start_method": (
                str(args.synthetic_start_method) if worker_count > 1 else "serial"
            ),
            "synthetic_case_count": len(synthetic_events),
            "failure_count": len(synthetic_failures),
            "records": synthetic_events,
            "failures": synthetic_failures,
        }
        save_pickle_data(synthetic_payload, synthetic_summary_path)
        print(f"Saved global synthetic sweep: {synthetic_summary_path}")
    ...
    summary = {
        ...
        "minimum_detected_frames": int(args.minimum_detected_frames),
        "camera_detection_definition": (
            f"at least {int(args.minimum_detected_frames)} sampled frames "
            "with m_app <= camera LM"
        ),
        "failure_count": len(failures),
        "events": events,
        "failures": failures,
        "synthetic_speed_sweep_enabled": bool(args.synthetic_speed_sweep),
        "synthetic_speed_values_kms": synthetic_speed_values.tolist(),
        ...
        "outputs": {
            ...
            "likelihood_plot": str(likelihood_plot_path),
            "likelihood_model_pickle": str(likelihood_model_path),
            "likelihood_csv": str(likelihood_csv_path),
            "synthetic_speed_summary_pickle": str(synthetic_summary_path),
            "synthetic_speed_csv": str(synthetic_csv_path),
            "synthetic_speed_plot": str(synthetic_plot_path),
            "synthetic_likelihood_plot": str(synthetic_likelihood_plot_path),
            "synthetic_likelihood_model_pickle": str(synthetic_likelihood_model_path),
            "synthetic_likelihood_csv": str(synthetic_likelihood_csv_path),
        },
    }
    ...
    if events:
        plot_batch_velocity_mass_summary(
            events, batch_plot_path,
            minimum_detected_frames=int(args.minimum_detected_frames),
        )
        ...
        try:
            likelihood_model = fit_detection_likelihood_model(
                events,
                minimum_detected_frames=int(args.minimum_detected_frames),
            )
            with likelihood_model_path.open("wb") as fh:
                pickle.dump(
                    likelihood_model, fh, protocol=pickle.HIGHEST_PROTOCOL
                )
            plot_detection_likelihood_surface(
                events,
                likelihood_model,
                likelihood_plot_path,
                limiting_apparent_magnitude=args.likelihood_limiting_mag,
                annotate_frame_counts=bool(args.likelihood_annotate_frames),
            )
            save_detection_likelihood_csv(
                events,
                likelihood_model,
                likelihood_csv_path,
                limiting_apparent_magnitude=args.likelihood_limiting_mag,
            )
            summary["detection_likelihood_model"] = likelihood_model
            save_pickle_data(summary, summary_path)
            print(f"Saved likelihood plot: {likelihood_plot_path}")
            print(f"Saved likelihood model: {likelihood_model_path}")
            print(f"Saved likelihood CSV: {likelihood_csv_path}")
            print(
                f"Camera detection uses >= {int(args.minimum_detected_frames)} frames. "
                "Empirical luminous proxy: mass * speed^"
                f"{likelihood_model['equivalent_speed_exponent_gamma']:.3f}; "
                f"residual scatter="
                f"{likelihood_model['residual_sigma_mag']:.3f} mag."
            )
        except Exception as exc:
            print(f"Could not create detection-likelihood products: {exc}")

    if synthetic_events:
        try:
            save_batch_summary_csv(synthetic_events, synthetic_csv_path)
            plot_batch_velocity_mass_summary(
                synthetic_events,
                synthetic_plot_path,
                annotate_frame_counts=False,
                title="Synthetic Mars-speed sweep detectability",
                minimum_detected_frames=int(args.minimum_detected_frames),
            )

            expanded_events = list(events) + list(synthetic_events)
            synthetic_model = fit_detection_likelihood_model(
                expanded_events,
                minimum_detected_frames=int(args.minimum_detected_frames),
            )
            with synthetic_likelihood_model_path.open("wb") as fh:
                pickle.dump(
                    synthetic_model, fh, protocol=pickle.HIGHEST_PROTOCOL
                )
            plot_detection_likelihood_surface(
                expanded_events,
                synthetic_model,
                synthetic_likelihood_plot_path,
                limiting_apparent_magnitude=args.likelihood_limiting_mag,
                annotate_frame_counts=False,
            )
            save_detection_likelihood_csv(
                expanded_events,
                synthetic_model,
                synthetic_likelihood_csv_path,
                limiting_apparent_magnitude=args.likelihood_limiting_mag,
            )
            summary["synthetic_detection_likelihood_model"] = synthetic_model
            save_pickle_data(summary, summary_path)
            print(f"Saved synthetic speed CSV: {synthetic_csv_path}")
            print(f"Saved synthetic velocity-mass plot: {synthetic_plot_path}")
            print(
                "Saved expanded likelihood surface: "
                f"{synthetic_likelihood_plot_path}"
            )
            print(
                f"Expanded camera detection uses >= {int(args.minimum_detected_frames)} frames. "
                "Empirical luminous proxy: mass * speed^"
                f"{synthetic_model['equivalent_speed_exponent_gamma']:.3f}; "
                f"residual scatter="
                f"{synthetic_model['residual_sigma_mag']:.3f} mag."
            )
        except Exception as exc:
            print(f"Could not create synthetic speed-sweep products: {exc}")
