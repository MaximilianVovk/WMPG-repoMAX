...
def plot_dynesty(dynesty_run_results, obs_data, flags_dict, fixed_values, output_folder='', file_name='', log_file='', cores=None, save_backup=False, finish_run=False):
    ...
    if save_backup:
        ...
        # finish_run when true means has just finish a run and must save the backup
    ...
    for input_dirfile in cml_args.input_dir:
        ...
        for i, (base_name, dynesty_info, prior_path, out_folder, report_txt) in enumerate(zip(
            ...
            if not cml_args.only_plot: 
                ...
                # Run dynesty
                try:
                    main_dynestsy(dynesty_file, obs_data, bounds, flags_dict, fixed_values, cml_args.cores, output_folder=out_folder, base_name=base_name, log_file_path=log_file_path, pool_MPI=pool_MPI, save_backup=save_backup)
                except Exception as e:
                    ...
                    try:
                        plot_dynesty(dsampler_results, obs_data, flags_dict, fixed_values, out_folder, base_name,log_file_path,cml_args.cores,save_backup=save_backup)
                    except Exception as e:
                        with open(log_file_path, "a") as log_file:
...
def main_dynestsy(dynesty_file, obs_data, bounds, flags_dict, fixed_values, n_core=1, output_folder="", base_name="",log_file_path="", pool_MPI=None, save_backup=True):
    """
    ...
    plot_dynesty(dsampler.results, obs_data, flags_dict, fixed_values, output_folder, base_name, log_file_path, n_core, save_backup=save_backup, finish_run=True)
